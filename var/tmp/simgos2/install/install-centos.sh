...
if [ -f /var/lib/yum/rpmdb_lock.pid ]; then
    sudo mv /var/lib/yum/rpmdb_lock.pid /var/lib/yum/rpmdb_lock.pid.bcp
fi
...
# Jika sudah install php dan versi 7.2 maka lakukan upgrade ke versi 7.4
...
if [ "$installed_php" != "" ]; then
    ...
    php_version=`rpm -qa php | grep "7\.2" | sed -e 's|.*p-\(.*\)-.*|\1|g'`
    if [ "$php_version" != "" ]; then
        echo "CentOS 7: Mulai" > /var/log/simgos2.log
        
        sudo yes | cp /var/tmp/simgos2/install/config/headers.conf /etc/httpd/conf.d/
        sudo rm -rf /etc/httpd/conf.d/welcome.conf
        find /etc/httpd/conf.d -name "*.conf" -exec sh -c 'x={}; sed -i -e "s/Indexes //g" "$x"' \;

        sudo sed -i '$ a Upgrade PHP 7.2 to 7.4' /var/log/simgos2.log
        cd /var/www/html/production/webapps/scripts/migrations
        sudo ./php_72_to_74.sh
        sudo sed -i 's/Upgrade PHP 7.2 to 7.4/Upgrade PHP 7.2 to 7.4 ......... OK/g' /var/log/simgos2.log
        ...
        if [ "$rpm" != "" ]; then
            ...
            cd /var/tmp/simgos2/install
            sudo sed -i -e 's/\r$//' install_grpc.sh
...
sudo yum -y install yum-utils nano wget unzip policycoreutils-python
...
if [ "$installed_remi" = "" ]; then
    sudo yum -y install https://rpms.remirepo.net/enterprise/remi-release-7.9.rpm
    sudo yum-config-manager --enable remi-php74
fi
installed_php=`sudo rpm -qa php`
if [ "$installed_php" = "" ]; then
    sudo yum -y install php-common php php-cli php-pdo php-mysqlnd php-opcache php-fpm php-soap php-intl php-pear php-bcmath php-gd php-json php-pecl-mcrypt php-mbstring php-pecl-ssh2 php-pecl-grpc php-pecl-protobuf
fi
...
#    wget -q https://dev.mysql.com/get/mysql80-community-release-el7-3.noarch.rpm
    ...
    sudo rpm -i mysql80-community-release-el7-5.noarch.rpm
fi
...
if [ "$installed_xtrabackup" = "" ]; then
    sudo yum -y install percona-xtrabackup-80
fi
...
sudo yes | cp install/config/centos7/selinux /etc/sysconfig/
sudo sed -i -e 's/\r$//' /etc/sysconfig/selinux

sudo yes | cp install/config/centos7/sudoers /etc/
sudo sed -i -e 's/\r$//' /etc/sudoers

sudo yes | cp install/config/centos7/00-base.conf /etc/httpd/conf.modules.d/
sudo yes | cp install/config/centos7/httpd.conf /etc/httpd/conf/
sudo yes | cp install/config/centos7/php.conf /etc/httpd/conf.d/
sudo yes | cp install/config/alias.conf /etc/httpd/conf.d/
sudo yes | cp install/config/headers.conf /etc/httpd/conf.d/
sudo yes | cp install/config/centos7/www.conf /etc/php-fpm.d/
...
if [ -f /var/lib/yum/rpmdb_lock.pid.bcp ]; then
    sudo mv /var/lib/yum/rpmdb_lock.pid.bcp /var/lib/yum/rpmdb_lock.pid
fi