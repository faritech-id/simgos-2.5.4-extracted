...
DROP PROCEDURE IF EXISTS `LaporanFarmasiTermahal`;
-- Dumping structure for procedure laporan.LaporanFarmasiTermahal
DELIMITER //
CREATE PROCEDURE `LaporanFarmasiTermahal`(
	IN `TGLAWAL` DATETIME,
   ...
   SET vKATEGORIBARANG = CONCAT(KATEGORIBARANG,'%');
	...
	SET @sqlText = CONCAT('
		SELECT inst.PPK, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST,lf.ID NOMOR, lf.KUNJUNGAN, lf.TANGGAL, master.getNamaLengkapPegawai(mp.NIP) NAMADOKTER
		   , CONCAT(''LAPORAN PELAYANAN '',UPPER(jk.DESKRIPSI),'' 50 PASIEN TERMAHAL'') JENISLAPORAN
			, rg.ID, rg.DESKRIPSI RUANG, r.ID, r.DESKRIPSI ASALPENGIRIM, master.getNamaLengkap(ps.NORM) NAMAPASIEN, DATE_FORMAT(ps.TANGGAL_LAHIR,''%d-%m-%Y'') TGLLAHIR
			, ps.ALAMAT,LPAD(ps.NORM,8,''0'') NORM
			, CONCAT(''RESEP '',UPPER(jenisk.DESKRIPSI)) JENISRESEP, IFNULL(master.getHeaderKategoriBarang(IF(''',KATEGORIBARANG,'''!=''0'',''',KATEGORIBARANG,''',IF(''',KATEGORI,'''!=''0'',''',KATEGORI,''',''',JENISINVENTORY,'''))),''Semua'') KATEGORI
			, pj.JENIS, crbyr.DESKRIPSI CARA_BYR
			, ib.NAMA NAMAOBAT, lf.JUMLAH JMLOBAT, rt.JUMLAH,  
			rt.TARIF, SUM(lf.JUMLAH * rt.TARIF) TOTAL,  CONCAT(lf.RACIKAN,lf.GROUP_RACIKAN) RACIKAN, lf.`STATUS` STATUSLAYANAN
