...
BEGIN
	...
	, rtd.OBAT_DARI_LUAR
--	, b.NAMA OBAT 
	, if(b.ID IS NULL, rtd.OBAT_DARI_LUAR,b.NAMA) OBAT
	, rtd.DOSIS, rtd.FREKUENSI KD_F , f.FREKUENSI DESC_F, rtd.RUTE KD_R, mr.DESKRIPSI DESC_R,  rtd.TINDAK_LANJUT KD_TL, tl.DESKRIPSI DESC_TL, rtd.PERUBAHAN_ATURAN_PAKAI KD_PAP, f2.FREKUENSI DESC_PAP
	...
	LEFT JOIN aplikasi.pengguna mp ON mp.ID=rt.OLEH
	...
	LEFT JOIN inventory.barang b ON b.ID=rtd.OBAT_DARI_LUAR
	, (SELECT mp.NAMA, ai.PPK, mp.ALAMAT
			            FROM aplikasi.instansi ai
			              , master.ppk mp
			            WHERE ai.PPK=mp.ID) inst
	
	WHERE rt.ID=rtd.REKONSILIASI_TRANSFER AND pk.NOPEN=pp.NOMOR AND pk.NOMOR=rt.KUNJUNGAN
