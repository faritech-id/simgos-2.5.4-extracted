...
BEGIN
	...
	, rdt.OBAT_DARI_LUAR KD_OBAT
--	, b.NAMA OBAT
	, if(b.ID IS NULL, rdt.OBAT_DARI_LUAR,b.NAMA) OBAT
	, rdt.DOSIS DOSIS, rdt.FREKUENSI KD_F, f.FREKUENSI DESC_F
	...
	LEFT JOIN aplikasi.pengguna mp ON mp.ID=rd.OLEH
	...
	LEFT JOIN inventory.barang b ON b.ID=rdt.OBAT_DARI_LUAR
	, (SELECT mp.NAMA, ai.PPK, mp.ALAMAT
			            FROM aplikasi.instansi ai
			              , master.ppk mp
			            WHERE ai.PPK=mp.ID) inst
	WHERE rd.ID=rdt.REKONSILIASI_DISCHARGE AND pp.NOMOR=rd.PENDAFTARAN AND pp.NOMOR=pk.NOPEN AND pk.NOMOR=rd.KUNJUNGAN
