...
-- Dumping structure for procedure laporan.LaporanPelayananPaketFarmasiPerPasien
DROP PROCEDURE IF EXISTS `LaporanPelayananPaketFarmasiPerPasien`;
DELIMITER //
CREATE PROCEDURE `LaporanPelayananPaketFarmasiPerPasien`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
   ...
   SET@sqlText = CONCAT ('
		...
		, pp.NOMOR, `master`.getNamaLengkap(pp.NORM) NMPASIEN, pk2.NOMOR, pp.NOMOR NOPEN
		, inst.PPK, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST
		, CONCAT(''LAPORAN PELAYANAN PAKET FARMASI PER PASIEN'') JENISLAPORAN
		...
		WHERE pk.REF=res.NOMOR AND r.ID=pk.RUANGAN AND r.JENIS=5 AND f.TINDAKAN_PAKET=1 AND pk.NOMOR=f.KUNJUNGAN AND f.FARMASI=b.ID
			AND pk2.`STATUS`!=0 AND f.`STATUS`=2
			AND r.JENIS_KUNJUNGAN=',LAPORAN,' AND pp.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,'''
