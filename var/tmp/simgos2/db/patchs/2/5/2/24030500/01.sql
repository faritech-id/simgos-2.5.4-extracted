...
-- Dumping structure for procedure laporan.LaporanPelayananResepRacikan
DROP PROCEDURE IF EXISTS `LaporanPelayananResepRacikan`;
DELIMITER //
CREATE PROCEDURE `LaporanPelayananResepRacikan`(
	IN `TGLAWAL` DATETIME,
   ...
   SET vKATEGORIBARANG = CONCAT(KATEGORIBARANG,'%');
	...
	SET @sqlText = CONCAT('
		SELECT inst.PPK, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST,lf.ID NOMOR, lf.KUNJUNGAN, lf.TANGGAL, master.getNamaLengkapPegawai(mp.NIP) NAMADOKTER
				, CONCAT(''LAPORAN PELAYANAN RESEP RACIKAN '',UPPER(jk.DESKRIPSI)) JENISLAPORAN
				, rg.DESKRIPSI RUANG, r.DESKRIPSI ASALPENGIRIM, master.getNamaLengkap(ps.NORM) NAMAPASIEN
				, LPAD(ps.NORM,8,''0'') NORM
				, master.getHeaderKategoriBarang(''',KATEGORI,''') KATEGORI, lf.FARMASI
				, ib.NAMA NAMAOBAT, lf.JUMLAH JMLOBAT, rt.JUMLAH,  rt.TARIF, (lf.JUMLAH * rt.TARIF) TOTAL
				, CONCAT(lf.RACIKAN,lf.GROUP_RACIKAN) RACIKAN, lf.`STATUS` STATUSLAYANAN, lf.DOSIS
				...
				, ref1.DESKRIPSI REF_PETUNJUK_RACIKAN
		...
		FROM layanan.farmasi lf
			  ...
			  LEFT JOIN `master`.referensi ref1 ON ref1.ID=lf.PETUNJUK_RACIKAN AND ref1.JENIS=84
		   ...
		   , pendaftaran.pendaftaran pp
			...
			WHERE  lf.`STATUS`=2 AND lf.KUNJUNGAN=pk.NOMOR AND pk.`STATUS` IN (1,2) AND lf.RACIKAN=1
