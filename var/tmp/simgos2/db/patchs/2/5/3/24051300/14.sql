...
-- Dumping structure for procedure kemkes-ihs.tindakanRadToServiceRequest
DROP PROCEDURE IF EXISTS `tindakanRadToServiceRequest`;
DELIMITER //
CREATE PROCEDURE `tindakanRadToServiceRequest`()
BEGIN
	...
	SELECT DISTINCT tm.ID, k.NOPEN, hrad.TANGGAL
	FROM layanan.tindakan_medis tm 
	LEFT JOIN layanan.hasil_rad hrad ON hrad.TINDAKAN_MEDIS = tm.ID
	LEFT JOIN `master`.tindakan t ON t.ID = tm.TINDAKAN
	LEFT JOIN `kemkes-ihs`.tindakan_to_loinc ttl ON ttl.TINDAKAN = t.ID
	LEFT JOIN pendaftaran.kunjungan k ON k.NOMOR = tm.KUNJUNGAN
 	LEFT JOIN pendaftaran.pendaftaran pn ON pn.NOMOR = k.NOPEN		  	 
 	LEFT JOIN pendaftaran.tujuan_pasien tp ON tp.NOPEN = pn.NOMOR
	LEFT JOIN `master`.ruangan r ON r.ID = k.RUANGAN
	LEFT JOIN `master`.ruangan r2 ON r2.ID = tp.RUANGAN
	, `kemkes-ihs`.sinkronisasi s
	WHERE s.ID = 15
	AND t.JENIS = 7 
	AND ttl.TINDAKAN IS NOT NULL 
	AND ttl.`STATUS` = 1
	AND hrad.`STATUS` = 2
	AND r2.JENIS_KUNJUNGAN = 1
	AND r.JENIS_KUNJUNGAN = 5
	AND tm.`STATUS` != 0
	AND k.`STATUS` != 0
	AND hrad.TANGGAL  > s.TANGGAL_TERAKHIR
	AND s.`STATUS` = 1
	ORDER BY hrad.TANGGAL;
