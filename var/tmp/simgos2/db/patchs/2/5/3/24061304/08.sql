...
BEGIN
	SELECT INST.NAMAINST, INST.ALAMATINST, INST.KOTA, INST.ID ID_PPK,
		   ...
		   , DATE_FORMAT(pp.TANGGAL,'%d-%m-%Y %H:%i:%s') TGLMASUK, DATE_FORMAT(lpp.TANGGAL,'%d-%m-%Y %H:%i:%s') TGLKELUAR
			, DATE_FORMAT(lpm.TANGGAL,'%d-%m-%Y') TGLMENINGGAL, DATE_FORMAT(lpm.TANGGAL,'%Y') THNMENINGGAL, DATE_FORMAT(lpm.TANGGAL,'%H:%i:%s') JAMMENINGGAL
			, cr.DESKRIPSI CARAKELUAR
			, kd.DESKRIPSI KEADAANKELUAR, UPPER(r.DESKRIPSI) UNIT, IFNULL(ksr.KODE_SURAT,'      ') KODE_SURAT, lpm.NOMOR NOMORLPM,
	...
	FROM layanan.pasien_meninggal lpm
		  ...
		  LEFT JOIN master.referensi pjm ON pj.JENIS=pjm.ID AND pjm.JENIS=10
