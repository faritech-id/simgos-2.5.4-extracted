...
-- Dumping structure for procedure medicalrecord.getPersentaseDiagnosaKeperawatanFaktorResiko
DROP PROCEDURE IF EXISTS `getPersentaseDiagnosaKeperawatanFaktorResiko`;
DELIMITER //
CREATE PROCEDURE `getPersentaseDiagnosaKeperawatanFaktorResiko`(
	IN `PFAKTORRESIKO` INT
)
BEGIN
	DECLARE VCONIND TEXT;
	DECLARE VIN TEXT;
	
	SET VIN = CONCAT('AND (a.JENIS = 10 AND a.INDIKATOR IN(',PFAKTORRESIKO,'))');
	
	SET VCONIND = PFAKTORRESIKO;
	...
	WHERE b.JENIS = 10 AND b.`STATUS` = 1 GROUP BY b.DIAGNOSA');
