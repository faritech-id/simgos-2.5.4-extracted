...
BEGIN
  ...
  SET vTANGGAL = DATE(TANGGAL);
  SET vTANGGALAWAL = CONCAT(DATE(TANGGAL),' 00:00:00');
  SET vTANGGALAKHIR = CONCAT(DATE(TANGGAL),' 23:59:59');
 ...
 SELECT ab.*, u.DESKRIPSI UNIT, ins.DESKRIPSI INSTALASI
	...
	SELECT LPAD(ps.NORM,8,''0'') NORM, pp.NOMOR NOPEN, master.getNamaLengkap(ps.NORM) NAMALENGKAP, CONCAT(DATE_FORMAT(ps.TANGGAL_LAHIR,''%d-%m-%Y''),'' ('',master.getCariUmur(pp.TANGGAL,ps.TANGGAL_LAHIR),'')'') TANGGAL_LAHIR, crbyr.DESKRIPSI CARABAYAR,
		    ...
		    , IF(hg.NOPEN IS NULL, ''Belum Grouping'',IF(hg.`STATUS`=1,''Sudah Final'',''Belum Final'')) STATUS_GROUPING
		     , IF(hg.NOPEN IS NULL, 1,IF(hg.`STATUS`=1,3,2)) ID_STATUS_GROUPING
		    , IF(hg.NOPEN IS NULL, '''',hg.TANGGAL) TANGGAL_GROUPING, tg.TAGIHAN
			 , hg.CODECBG
			 , REVERSE(LEFT(REVERSE(hg.CODECBG), INSTR(REVERSE(hg.CODECBG),''-'')-1)) LEVEL
			 , hg.TARIFCBG, cbg.DESKRIPSI DESKRIPSICBG
		...
		#	 , IF(@TARIFCBG=0,0,IF((@TARIFRS/@TARIFCBG) * 100 <= 75, 1, IF((@TARIFRS/@TARIFCBG) * 100 <= 90, 2, 3))) IDTARIF
			 , IF(@TARIFCBG=0 OR @TARIFCBG IS NULL ,0,IF((@TARIFRS/@TARIFCBG) * 100 > 90, 3, IF((@TARIFRS/@TARIFCBG) * 100 > 75, 2, 1))) IDTARIF
	...
	WHERE pk.`STATUS` IN (1,2) AND pk.NOPEN=pp.NOMOR
	   ...
	   ',IF(RUANGAN=0 OR RUANGAN=1,'',CONCAT(' AND pk.RUANGAN LIKE ''',vRUANGAN,''' ')),'
		...
		',IF(PSEVERITY='0','',CONCAT(' AND REVERSE(LEFT(REVERSE(CODECBG), INSTR(REVERSE(CODECBG),''-'')-1))=''',PSEVERITY,'''')),'
	   AND pk.MASUK < DATE_ADD(''',vTANGGALAWAL,''',INTERVAL 1 DAY)
		AND (pk.KELUAR > ''',vTANGGALAKHIR,''' OR pk.KELUAR IS NULL)
	   AND pp.`STATUS` !=0
	 ) ab 
	...
	',IF(STATUSGROUPING=0,'',CONCAT(' AND ID_STATUS_GROUPING=',STATUSGROUPING)),'
