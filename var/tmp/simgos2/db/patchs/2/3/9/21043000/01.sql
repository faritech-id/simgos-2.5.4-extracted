...
DELIMITER //
CREATE PROCEDURE `listRuangKamarTidur`(
	IN `PRUANGAN` CHAR(10),
	IN `PKAMAR` SMALLINT,
	IN `PKELAS` SMALLINT,
	IN `PSTATUS` INT,
...
BEGIN
	...
	SET @sqlText = CONCAT('
		SELECT rkt.ID, rgn.ID RUANGAN_ID, rgn.DESKRIPSI RUANGAN, rk.ID KAMAR_ID, rk.KAMAR, rkls.DESKRIPSI KELAS, rkls.ID KELAS_ID, rkt.TEMPAT_TIDUR, rkt.NORM, rkt.NAMA, rkt.NOPEN
	 ...
	 IF(PKELAS = 0, '', CONCAT(' AND rkls.ID = ', PKELAS)),
