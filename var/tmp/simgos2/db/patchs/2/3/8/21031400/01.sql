...
-- Dumping structure for procedure informasi.infoRuangKamar
DROP PROCEDURE IF EXISTS `infoRuangKamar`;
DELIMITER //
CREATE PROCEDURE `infoRuangKamar`(IN `PRUANGAN` LONGTEXT)
BEGIN
	SET @sqlText = CONCAT('
		SELECT r.ID ID_RUANGAN, ru.DESKRIPSI NM_RUANGAN, r.KAMAR NM_KAMAR,  ref.DESKRIPSI NM_KELAS
		FROM master.ruang_kamar r, master.ruang_kamar_tidur rkt, master.referensi ref, master.ruangan ru
		WHERE rkt.RUANG_KAMAR = r.ID AND ru.ID = r.RUANGAN AND ref.ID = r.KELAS AND ref.JENIS = 19 AND rkt.STATUS NOT IN (0) AND  r.STATUS=1 AND r.RUANGAN IN (',PRUANGAN,')
...
-- Dumping structure for procedure informasi.infoRuangKamarTidur
...
-- Dumping structure for procedure informasi.InfoRuangKamarTidurDetil
DROP PROCEDURE IF EXISTS `InfoRuangKamarTidurDetil`;
DELIMITER //
CREATE PROCEDURE `InfoRuangKamarTidurDetil`(IN `PKAMAR` LONGTEXT)
BEGIN
