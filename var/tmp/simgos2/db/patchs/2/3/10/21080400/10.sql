...
DROP TRIGGER IF EXISTS `sitt_before_insert`;
...
-- membuang struktur untuk trigger kemkes.pasien_tb_before_update
DROP TRIGGER IF EXISTS `sitt_before_update`;
DROP TRIGGER IF EXISTS `pasien_tb_before_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `pasien_tb_before_update` BEFORE UPDATE ON `pasien_tb` FOR EACH ROW BEGIN
	DECLARE VTIPE, VJML TINYINT;
