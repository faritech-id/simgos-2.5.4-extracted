...
-- membuang struktur untuk procedure pembayaran.CetakSetoranKasirPenjualanNonTunai
...
-- membuang struktur untuk function pembayaran.getTotalNonTunai
DROP FUNCTION IF EXISTS `getTotalNonTunai`;
DELIMITER //
CREATE FUNCTION `getTotalNonTunai`(
	`PTAGIHAN` CHAR(10)
...
BEGIN
	...
	SELECT SUM(TOTAL) INTO VTOTAL 
	 ...
	 WHERE TAGIHAN = PTAGIHAN
	   AND NOT JENIS_LAYANAN_ID IN (1) # Bukan Tunai
	   AND NOT STATUS IN (0);
