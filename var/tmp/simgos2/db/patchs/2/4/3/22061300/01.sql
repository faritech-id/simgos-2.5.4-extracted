...
-- Dumping database structure for inacbg
CREATE DATABASE IF NOT EXISTS `inacbg`;
USE `inacbg`;

-- Dumping structure for procedure inacbg.CetakLapIndividual5
...
SET @sqlText = CONCAT('	
	SELECT inst.KODERS, inst.KELASRS, UPPER(inst.NAMA) NAMAINSTANSI, INSERT(INSERT(INSERT(LPAD(p.NORM,8,''0''),3,0,''-''),6,0,''-''),9,0,''-'') NORM, master.getNamaLengkap(p.NORM) NAMALENGKAP
		...
		, (SELECT md.KODE FROM medicalrecord.diagnosa md WHERE md.UTAMA=1 AND md.`STATUS`=1 AND md.INA_GROUPER=0 AND md.NOPEN=''',PNOPEN,''') KODEDIAGNOSAUTAMA
		, (SELECT mr.STR
					FROM master.mrconso mr,
						  medicalrecord.diagnosa md
					WHERE mr.CODE=md.KODE AND md.UTAMA=1 AND md.`STATUS`=1 AND md.NOPEN=''',PNOPEN,'''
					  AND mr.SAB=''ICD10_1998'' AND TTY IN (''PX'', ''PT'') AND mr.VERSION!=4 AND md.INA_GROUPER=0
