...
DELIMITER //
CREATE EVENT `autoExecuteIhsTransaction` ON SCHEDULE EVERY 1 MINUTE STARTS '2022-07-25 20:20:07' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN	
	IF EXISTS(SELECT * FROM aplikasi.integrasi integ WHERE integ.ID = 7 AND integ.`STATUS` = 1) THEN
		...
		CALL `kemkes-ihs`.pendaftaranToConsent();
	END IF;
END//
DELIMITER ;
