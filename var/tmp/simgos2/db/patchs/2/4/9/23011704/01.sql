...
SET @sqlText = CONCAT('	
	SELECT inst.KODERS, inst.KELASRS, UPPER(inst.NAMA) NAMAINSTANSI, INSERT(INSERT(INSERT(LPAD(p.NORM,8,''0''),3,0,''-''),6,0,''-''),9,0,''-'') NORM, master.getNamaLengkap(p.NORM) NAMALENGKAP
		...
		, IF(r.JENIS_KUNJUNGAN=3, DATE_FORMAT(aplikasi.getValuePropertyJSON(gr.DATA,''tgl_keluar''),''%d/%m/%Y'') , pd.TANGGAL) TGLKELUAR
...
'')
		...
		, IF(IF(pw.PROFESI=7,'''', `master`.getNamaLengkapPegawai(pw.NIP)) IS NULL OR IF(pw.PROFESI=7,'''', `master`.getNamaLengkapPegawai(pw.NIP))='''', '''', CONCAT(''Verifikator: '',IF(pw.PROFESI=7,'''', `master`.getNamaLengkapPegawai(pw.NIP)))) VERIFIKATOR
		, IF(`master`.getNamaLengkapPegawai(pwc.NIP) IS NULL, '''', CONCAT(''Coder: '', `master`.getNamaLengkapPegawai(pwc.NIP))) CODER	
	   , IF(IF(r.JENIS_KUNJUNGAN=3,rg.DESKRIPSI, r.DESKRIPSI) IS NULL,'''',CONCAT(''Ruang Rawat: '',IF(r.JENIS_KUNJUNGAN=3,rg.DESKRIPSI, r.DESKRIPSI))) RUANG_RAWAT
