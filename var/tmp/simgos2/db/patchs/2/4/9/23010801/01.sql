...
BEGIN
	SELECT inst.PPK ID_PPK, UPPER(inst.NAMA) NAMA_INSTANSI, inst.KOTA, inst.ALAMAT
		 ...
		 , CONCAT(pst.noKartu, '  ( MR. ',p.NORM,' )') NOMORKARTU, pst.norm NORMBPJS
	    ...
	    , r.DESKRIPSI RUANGAN
	    , master.getNamaLengkapPegawai(drk.NIP) DOKTER
	    , drskdp.nama DRSEP
	    ...
	    , CONCAT(DATE_FORMAT(pri.TANGGAL, '%d-%m-%Y')) TGLSO, pri.DESKRIPSI KETSO
	    ...
	    , IF(IF(rkbpjs.noSurat IS NULL, rkbpjs1.noSurat, rkbpjs.noSurat) IS NULL,'Nomor Surat BPJS tidak terbit', IF(rkbpjs.noSurat IS NULL, rkbpjs1.noSurat, rkbpjs.noSurat)) NOSBPJS
	    ...
	    , IFNULL(bk.noRujukan, '') NORJK
		 , IFNULL(DATE_FORMAT(bk.tglRujukan, '%d-%m-%Y'),'') TGLRJK
		 , IF(jk.RUANGAN IS NULL,DATE_FORMAT(DATE_ADD(bk.tglRujukan, INTERVAL 90 DAY), '%d-%m-%Y'),'1 Kali Kunjungan Setelah Rawat Inap') MASABERLAKU
