			...
			IF PJENIS = 3 OR PJENIS = 2 THEN
				...
				BEGIN
					...
					IF VEKSEKUSI = 1 THEN
						...
						# STORE TOTAL TAGIHAN VIP KE PENJAMIN TAGIHAN				
						UPDATE pembayaran.penjamin_tagihan pt
		...
		BEGIN
			...
			BEGIN					
				...
				BEGIN
					...
					# BUKAN MANUAL PERSENTASE NAIK VIP
					IF NOT EXISTS(SELECT 1 FROM aplikasi.properti_config pc WHERE pc.ID = 20 AND pc.VALUE = 'TRUE') THEN
						...
						UPDATE pembayaran.penjamin_tagihan pt
						 ...
						 WHERE pt.TAGIHAN = PTAGIHAN
				   		AND pt.PENJAMIN = 2;
				   ELSE
				   	# PENENTUAN PERSENTASE ATAU TAGIHAN VIP MANA YANG LEBIH KECIL MAKA PERSENTASE MANUAL TIDAK BERLAKU
				   	IF EXISTS(SELECT 1
						  FROM aplikasi.properti_config pc
						 WHERE pc.ID = 57
						   AND VALUE = 'TRUE') THEN
...
-- membuang struktur untuk trigger pembayaran.penjamin_tagihan_after_update
DROP TRIGGER IF EXISTS `penjamin_tagihan_after_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `penjamin_tagihan_after_update` AFTER UPDATE ON `penjamin_tagihan` FOR EACH ROW BEGIN
...
CREATE TRIGGER `penjamin_tagihan_before_update` BEFORE UPDATE ON `penjamin_tagihan` FOR EACH ROW BEGIN
	IF NEW.PERSENTASE_TARIF_INACBG_KELAS1 != OLD.PERSENTASE_TARIF_INACBG_KELAS1 OR NEW.TARIF_INACBG_KELAS1 != OLD.TARIF_INACBG_KELAS1 THEN
