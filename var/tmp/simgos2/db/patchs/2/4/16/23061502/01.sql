...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM aplikasi.properti_config p WHERE p.ID = 66 AND p.VALUE = 'TRUE') THEN	
		IF EXISTS(SELECT 1 FROM (
			SELECT (IF((SELECT inventory.getStokBarangRuangan(f.FARMASI, k.RUANGAN)) < 0, 0, (SELECT inventory.getStokBarangRuangan(f.FARMASI, k.RUANGAN))) - (f.JUMLAH - f.BON)) SELISIH
			FROM pendaftaran.kunjungan k, layanan.farmasi f
			where f.KUNJUNGAN = k.NOMOR AND (f.ALASAN_TIDAK_TERLAYANI IS NULL OR f.ALASAN_TIDAK_TERLAYANI ='') AND k.NOMOR = PKUNJUNGAN
