...
-- membuang struktur untuk procedure pembayaran.prosesPerhitunganJasaRaharja
DROP PROCEDURE IF EXISTS `prosesPerhitunganJasaRaharja`;
DELIMITER //
CREATE PROCEDURE `prosesPerhitunganJasaRaharja`(
	IN `PTAGIHAN` CHAR(10),
	IN `PREF_ID` CHAR(19),
	IN `PJENIS` TINYINT,
...
BEGIN
	DECLARE VMAKSIMAL_JAMINAN, VTOTAL_TAGIHAN DECIMAL(60,2);

    /*
     Ubah referensi jenis 10 untuk jasa raharja ref_id = 4.1 
     4 = akses procedure proses perhitungan yaitu prosesPerhitunganJasaRaharja
     1 = allow edit penginputan total penjamin di form tagihan
     */
	
	SET VMAKSIMAL_JAMINAN = 20000000;
	SET VTOTAL_TAGIHAN = pembayaran.getTotalTagihan(PTAGIHAN);
	
	IF VTOTAL_TAGIHAN > VMAKSIMAL_JAMINAN THEN
		SET VTOTAL_TAGIHAN = VMAKSIMAL_JAMINAN;
	END IF;
