...
-- Dumping structure for trigger pendaftaran.onAfterInsertKunjungan
...
-- Dumping structure for trigger pendaftaran.onAfterUpdateKunjungan
...
-- Dumping structure for trigger pendaftaran.pendaftaran_after_update
...
CREATE TRIGGER `pendaftaran_after_update` AFTER UPDATE ON `pendaftaran` FOR EACH ROW BEGIN
	...
	IF EXISTS(SELECT 1 FROM information_schema.ROUTINES r WHERE r.ROUTINE_SCHEMA = 'regonline' AND r.ROUTINE_NAME = 'insertTaskAntrian') THEN
		CALL regonline.insertTaskAntrian(OLD.NOMOR,'' ,1, NULL);
	END IF;
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;
