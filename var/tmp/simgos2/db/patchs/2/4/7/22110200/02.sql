...
USE `kemkes-ihs`;
-- Dumping structure for trigger kemkes-ihs.condition_before_update
DROP TRIGGER IF EXISTS `condition_before_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `condition_before_update` BEFORE UPDATE ON `condition` FOR EACH ROW BEGIN
	...
	IF OLD.send = 0 AND NEW.send = 1 AND OLD.id IS NULL THEN
		SELECT 
		 p.DIAGNOSA,
		 JSON_OBJECT('coding', 
		   JSON_ARRAY(
		 ...
		 WHERE p.ID = OLD.refId
		  AND p.STATUS = 1
		  ORDER BY mrc.VERSION DESC
		  LIMIT 1;
