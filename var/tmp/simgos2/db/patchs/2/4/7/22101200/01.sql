...
BEGIN
	SELECT inst.PPK ID_PPK, UPPER(inst.NAMA) NAMA_INSTANSI, inst.KOTA, inst.ALAMAT
	    ...
	    , CONCAT(DATE_FORMAT(jk.DIBUAT_TANGGAL, '%Y'),IF(jk.NOMOR IS NULL, pri.NOMOR, jk.NOMOR)) NOSURAT
	    ...
	    , IFNULL(bk.noRujukan, '') NORJK
		 ...
		 , IF(bk.jenisPelayanan=2,DATE_FORMAT(DATE_ADD(bk.tglRujukan, INTERVAL 90 DAY), '%d-%m-%Y'),'1 Kali Kunjungan Setelah Rawat Inap') MASABERLAKU
