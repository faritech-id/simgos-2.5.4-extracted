...
BEGIN
   ...
   WHERE r.NOPEN=PNOPEN AND UTAMA=PUTAMA;
	...
	SELECT d.NOPEN,  d.BARIS, p.TINDAKANMEDIS, p.IDPROSEDUR, fr.IDFARMASI
			, IF(@idg!=d.IDDIAGNOSA,dg.KODE, '') KODE_DIAGNOSA
			, IF(@idg!=d.IDDIAGNOSA,`master`.getDeskripsiICD(dg.KODE), '')  DESKRIPSI_DIAGNOSA
			, @idg:=d.IDDIAGNOSA IDDIAGNOSA
			, dg.DIAGNOSA
	...
	WHERE d.IDDIAGNOSA=dg.ID AND dg.`STATUS`!=0 AND dg.INA_GROUPER=0 AND dg.NOPEN=PNOPEN AND dg.UTAMA=PUTAMA;	 	
END//
DELIMITER ;
