...
BEGIN
	SET @sqlText = CONCAT('
		SELECT r.*, 
			 ...
			 IF(pt.TOTAL IS NULL,(ROUND(pembayaran.getTotalTagihanPembayaran(t.ID)) + ROUND(t.PEMBULATAN)), pt.TOTAL) TOTALJUMLAHBAYAR,
			 (IF(pt.TOTAL IS NULL, (ROUND(pembayaran.getTotalTagihanPembayaran(t.ID)) + ROUND(t.PEMBULATAN)), pt.TOTAL) - @tdp) JUMLAHBAYAR
			...
			, IF(pjt.NAIK_KELAS=1,(pjt.TOTAL_NAIK_KELAS - pjt.TOTAL)
			 ...
			 , w.DESKRIPSI WILAYAH, ROUND(t.PEMBULATAN) PEMBULATAN
		  ...
		  LEFT JOIN pendaftaran.pendaftaran pd ON tpd.PENDAFTARAN=pd.NOMOR AND pd.STATUS=2
