...
BEGIN
	...
	SELECT CONCAT(''PERIODE : '',  DATE_FORMAT(MIN(BUKA),''%d-%m-%Y %H:%i:%s''), '' s/d '', DATE_FORMAT(MAX(TUTUP),''%d-%m-%Y %H:%i:%s''))
	  INTO @PERIODE
	  FROM pembayaran.transaksi_kasir sk WHERE NOMOR IN (',PNOMOR,')'); -- 316, 317
	...
	SELECT *
	  FROM (
		SELECT @NAMAINST NAMAINST, @ALAMATINST ALAMATINST, master.getNamaLengkapPegawai(mp.NIP) KASIR, tk.BUKA, tk.TUTUP, tk.`STATUS`,
