...
SELECT INST.*
	  ...
	  , `master`.getNamaLengkapPegawai(demg.NIP) DOKTER, CONCAT(n.TINGGI_BADAN, ' cm') TINGGI_BADAN, CONCAT(n.BERAT_BADAN, ' kg') BERAT_BADAN
	FROM medicalrecord.pemeriksaan_emg emg
		 LEFT JOIN `master`.dokter demg ON emg.OLEH = demg.ID
