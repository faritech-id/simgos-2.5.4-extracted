...
CREATE TRIGGER `hasil_log_after_insert` AFTER INSERT ON `hasil_log` FOR EACH ROW BEGIN   
	...
	IF NOT EXISTS(SELECT 1 FROM lis.parameter_lis p WHERE p.VENDOR_ID = NEW.VENDOR_LIS AND p.KODE = NEW.LIS_KODE_TEST LIMIT 1) THEN
		...
		SELECT 1 
		 ...
		 WHERE mh.VENDOR_LIS = NEW.VENDOR_LIS
		   AND mh.LIS_KODE_TEST = TRIM(NEW.LIS_KODE_TEST)
			...
			AND mh.HIS_KODE_TEST = NEW.HIS_KODE_TEST
		LIMIT 1) THEN
		INSERT INTO lis.mapping_hasil(`VENDOR_LIS`, LIS_KODE_TEST, PREFIX_KODE, HIS_KODE_TEST)
		VALUES(NEW.VENDOR_LIS, TRIM(NEW.LIS_KODE_TEST), NEW.PREFIX_KODE, NEW.HIS_KODE_TEST); 
	END IF;
	...
	IF NOT EXISTS(
		SELECT 1 
		 ...
		 WHERE mh.VENDOR_LIS = NEW.VENDOR_ID
		   AND mh.LIS_KODE_TEST = TRIM(NEW.KODE)
		LIMIT 1) THEN
		INSERT INTO lis.mapping_hasil(`VENDOR_LIS`, LIS_KODE_TEST)
		VALUES(NEW.VENDOR_ID, TRIM(NEW.KODE)); 
	END IF;
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;
