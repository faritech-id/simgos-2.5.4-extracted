...
-- membuang struktur untuk trigger pembayaran.pembayaran_tagihan_before_update
DROP TRIGGER IF EXISTS `pembayaran_tagihan_before_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `pembayaran_tagihan_before_update` BEFORE UPDATE ON `pembayaran_tagihan` FOR EACH ROW BEGIN
	IF NEW.JENIS = 1 THEN
		IF OLD.STATUS != NEW.STATUS AND NEW.STATUS = 2 THEN
			IF EXISTS(
				SELECT 1 
				  FROM pembayaran.pembatalan_tagihan pt 
				 WHERE pt.TAGIHAN = OLD.TAGIHAN 
				   AND pt.JENIS = 1 
					AND pt.STATUS = 1) THEN
				SET NEW.REF = OLD.REF;
				SET NEW.TANGGAL = OLD.TANGGAL;
				SET NEW.TOTAL = OLD.TOTAL;
				SET NEW.TRANSAKSI_KASIR_NOMOR = OLD.TRANSAKSI_KASIR_NOMOR;
				SET NEW.OLEH = OLD.OLEH;
			END IF;
		END IF;
	END IF;
END//
