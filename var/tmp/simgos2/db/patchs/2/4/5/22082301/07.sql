...
-- Dumping structure for procedure pembayaran.storeFarmasi
DROP PROCEDURE IF EXISTS `storeFarmasi`;
DELIMITER //
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `storeFarmasi`(IN `PKUNJUNGAN` CHAR(19), IN `PLAYANAN_FARMASI` CHAR(11), IN `PFARMASI` SMALLINT, IN `PJUMLAH` DECIMAL(60,2)
)
BEGIN
