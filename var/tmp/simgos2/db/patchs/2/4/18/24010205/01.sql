...
DROP PROCEDURE IF EXISTS `LaporanObatStagnan`;
DELIMITER //
...
BEGIN
 ...
 SET @sqlText = CONCAT('
 	SELECT * FROM 
		(
			SELECT inst.*
		   , CONCAT (''LAPORAN OBAT STAGNAN '') JENISLAPORAN, master.getHeaderLaporan(''',RUANGAN,''') INSTALASI
	      , IFNULL(master.getHeaderKategoriBarang(IF(''',KATEGORIBARANG,'''!=''0'',''',KATEGORIBARANG,''',IF(''',KATEGORI,'''!=''0'',''',KATEGORI,''',''',JENISINVENTORY,'''))),''Semua'') KATEGORIBRG
	      ...
	      , IF(',BARANG,'=0,''Semua'',(SELECT br.NAMA FROM inventory.barang br WHERE br.ID=',BARANG,')) BARANGHEADER
			, LPAD(ib.ID,8,''0'')ID, ib.NAMA NAMAOBAT, DATE_FORMAT(pbd.MASA_BERLAKU,''%Y-%m-%d'')MASA_BERLAKU, k.NAMA NMKATEGORI, st.NAMA NMSATUAN, ref.DESKRIPSI NMMERK
		   , SUM(br.STOK) STOK, br.RUANGAN
		   , inventory.getHargaPenerimaanTerakhirBySo(ib.ID, so.ID) HARGAPENERIMAANTERAKHIR
			...
			WHERE sod.BARANG_RUANGAN=br.ID AND br.BARANG=ib.ID
				AND sod.STOK_OPNAME=so.ID AND so.STATUS!=0 
