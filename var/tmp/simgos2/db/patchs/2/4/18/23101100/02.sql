		...
		ELSE
			...
			ELSE
				...
				IF NOT VMODE IS NULL THEN
					...
					ELSEIF VMODE = 3 THEN
						CALL pembayaran.prosesPerhitunganBPJSMode3(PTAGIHAN, PREF_ID, PJENIS, PTOTAL, PINSERTED, PKELAS);
					ELSEIF VMODE = 4 THEN
						CALL pembayaran.prosesPerhitunganBPJSMode4(PTAGIHAN, PREF_ID, PJENIS, PTOTAL, PINSERTED, PKELAS);
					END IF;
				END IF;
			END IF;
		END IF;
...
-- membuang struktur untuk procedure pembayaran.prosesPerhitunganBPJSMode3
DROP PROCEDURE IF EXISTS `prosesPerhitunganBPJSMode3`;
DELIMITER //
CREATE PROCEDURE `prosesPerhitunganBPJSMode3`(
	IN `PTAGIHAN` CHAR(10),
	IN `PREF_ID` CHAR(19),
	IN `PJENIS` TINYINT,
	IN `PTOTAL` DECIMAL(60,2),
		...
		BEGIN
			...
			BEGIN					
				...
				BEGIN
			   	...
			   	IF VSELISIH > 0 THEN
			   		IF VSELISIH <= (VSELKELAS1KURANGHAKKELAS + VSELMAX) THEN
					   	SET VSELISIH_MINIMAL = IF(VGUNAKANPERSENTASE = 1, VSELMIN, VSELISIH);
					   ELSE
					   	SET VSELISIH_MINIMAL = IF(VGUNAKANPERSENTASE = 1, VSELMIN, (VSELKELAS1KURANGHAKKELAS + VSELMAX));
					   END IF;
				   ELSE
...
-- membuang struktur untuk procedure pembayaran.prosesPerhitunganBPJSMode4
DROP PROCEDURE IF EXISTS `prosesPerhitunganBPJSMode4`;
DELIMITER //
CREATE PROCEDURE `prosesPerhitunganBPJSMode4`(
	IN `PTAGIHAN` CHAR(10),
	IN `PREF_ID` CHAR(19),
	IN `PJENIS` TINYINT,
	IN `PTOTAL` DECIMAL(60,2),
		...
		BEGIN
			...
			BEGIN					
				...
				BEGIN
					...
					SET VSELKELAS1KURANGHAKKELAS = VTARIF_NAIK_KELAS - VTOTAL_JAMINAN;
					SET VSELMAX = VTARIF_INACBG_KELAS1 * ((VTOTAL_TAGIHAN_RS - VTARIF_INACBG_KELAS1) / VTARIF_INACBG_KELAS1);
					SET VSELMIN = VTARIF_NAIK_KELAS * (VMINTARIFINACBGPERSEN/100);
