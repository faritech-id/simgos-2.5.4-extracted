...
BEGIN
	...
	SELECT 
		IFNULL(SUM(pbc.SKOR_BALLANCER_CAIRAN), 0) INTO THARIINI
	FROM medicalrecord.penilaian_ballance_cairan pbc
	WHERE DATE(pbc.WAKTU_PEMERIKSAAN) = DATE(NOW()) AND pbc.`STATUS` = 1;
	
	SELECT 
		IFNULL(SUM(pbc.SKOR_BALLANCER_CAIRAN), 0) INTO TNOPEN
	FROM medicalrecord.penilaian_ballance_cairan pbc
	LEFT JOIN pendaftaran.kunjungan k ON k.NOMOR = pbc.KUNJUNGAN
	WHERE k.NOPEN = PNOPEN AND pbc.`STATUS` = 1;
