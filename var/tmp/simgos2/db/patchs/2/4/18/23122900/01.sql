...
-- Dumping structure for trigger master.onAfterUpdatePegawai
	...
	IF OLD.STATUS != NEW.STATUS AND NEW.STATUS = 1 THEN
		IF OLD.PROFESI != NEW.PROFESI THEN
			IF NEW.PROFESI = 4 THEN
				UPDATE `master`.dokter SET STATUS = 1 WHERE NIP = OLD.NIP;
			END IF;
			IF EXISTS(SELECT 1 FROM `master`.paramedis_map WHERE ID = NEW.PROFESI) THEN
				UPDATE `master`.perawat SET STATUS = 1 WHERE NIP = OLD.NIP;
			END IF;
			IF NOT EXISTS(SELECT 1 FROM `master`.paramedis_map WHERE ID = NEW.PROFESI) AND NEW.PROFESI != 4 THEN
				UPDATE `master`.staff SET STATUS = 1 WHERE NIP = OLD.NIP;
			END IF;
		ELSE
			IF OLD.PROFESI = 4 THEN
				UPDATE `master`.dokter SET STATUS = 1 WHERE NIP = OLD.NIP;
			END IF;
			IF EXISTS(SELECT 1 FROM `master`.paramedis_map WHERE ID = OLD.PROFESI) THEN
				UPDATE `master`.perawat SET STATUS = 1 WHERE NIP = OLD.NIP;
			END IF;
			IF NOT EXISTS(SELECT 1 FROM `master`.paramedis_map WHERE ID = OLD.PROFESI) AND OLD.PROFESI != 4 THEN
				UPDATE `master`.staff SET STATUS = 1 WHERE NIP = OLD.NIP;
			END IF;
		END IF;
	END IF;
