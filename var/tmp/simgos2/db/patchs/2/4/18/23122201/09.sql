...
USE laporan;
-- Dumping structure for procedure laporan.LaporanPengirimanAmpraRuangan
DROP PROCEDURE IF EXISTS `LaporanPengirimanAmpraRuangan`;
DELIMITER //
CREATE PROCEDURE `LaporanPengirimanAmpraRuangan`(
	IN `TGLAWAL` DATETIME,
   ...
   SET vKATEGORIBARANG = CONCAT(KATEGORIBARANG,'%');
	...
	SET @sqlText = CONCAT('
				SELECT 
							...
							, p2.TANGGAL TGL_MINTA , pd2.PERMINTAAN NMR_MINTA, pd2.BARANG ID_BRG_MINTA, minta.NAMA BRG_MINTA, pd2.JUMLAH JML_MINTA 
