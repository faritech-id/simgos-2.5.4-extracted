   ...
   SET vKATEGORIBARANG = CONCAT(KATEGORIBARANG,'%');
	...
	SET @sqlText = CONCAT('
				SELECT inst.PPK, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST
						, ib.ID , ib.NAMA NAMAOBAT, jts.DESKRIPSI, DATE_FORMAT(tsr.TANGGAL,''%d-%m-%Y %H:%i:%s'') TGLPELAYANAN
						...
						, LPAD(ps.NORM,8,''0'') NORM, pk.NOPEN, master.getNamaLengkap(ps.NORM) NAMAPASIEN, IF(tsr.JENIS IN (33,64), ra.DESKRIPSI, inventory.getKeteranganTransaksi(tsr.JENIS,tsr.REF)) RUANGASAL
					FROM inventory.transaksi_stok_ruangan tsr
					     LEFT JOIN inventory.jenis_transaksi_stok jts ON tsr.JENIS=jts.ID
					     LEFT JOIN layanan.layanan_bon_sisa_farmasi lb ON lb.ID = tsr.REF AND tsr.JENIS=64
						  LEFT JOIN layanan.bon_sisa_farmasi bon ON bon.ID=lb.REF
						  LEFT JOIN layanan.farmasi f ON f.ID = IFNULL(bon.REF,tsr.REF) 
					     LEFT JOIN pendaftaran.kunjungan pk ON f.KUNJUNGAN=pk.NOMOR
