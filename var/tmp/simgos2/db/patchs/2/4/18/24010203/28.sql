...
-- Dumping structure for trigger kemkes-ihs.medication_dispanse_before_update
DROP TRIGGER IF EXISTS `medication_dispanse_before_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `medication_dispanse_before_update` BEFORE UPDATE ON `medication_dispanse` FOR EACH ROW BEGIN
	...
	SELECT 
	 JSON_ARRAY(
		...
		`kemkes-ihs`.getIdentifierUrutanLayananResep(OLD.refId)
	 ),
	 org.id
	 INTO
	 VIDENTIFIER,
	...
	IF OLD.status_racikan = 0 THEN	
		...
		WHERE b.ID = OLD.barang AND m.refId = OLD.refId AND m.jenis = 2;
	ELSE
		SELECT
			JSON_OBJECT(
				'reference', CONCAT('Medication/', m.id)
		...
		WHERE m.group_racikan = OLD.group_racikan AND m.refId = OLD.refId AND m.jenis = 2;
	END IF;
	...
	IF OLD.status_racikan = 0 THEN
		SELECT
		 ...
		 WHERE o.KUNJUNGAN = OLD.refId AND o.FARMASI = OLD.barang LIMIT 1;
	ELSE
		SELECT
			k.NOPEN,
			`kemkes-ihs`.getPatient(p.NORM),
		 ...
		 LEFT JOIN pendaftaran.pendaftaran p ON p.NOMOR = k.NOPEN
		 LEFT JOIN `master`.frekuensi_aturan_resep far ON far.ID = o.FREKUENSI
		 WHERE o.KUNJUNGAN = OLD.refId AND o.GROUP_RACIKAN = OLD.group_racikan LIMIT 1; 	
	END IF;
