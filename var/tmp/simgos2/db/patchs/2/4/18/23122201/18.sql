...
USE laporan;
-- Dumping structure for procedure laporan.LaporanRekapPenerimaanBarangPerPbf
DROP PROCEDURE IF EXISTS `LaporanRekapPenerimaanBarangPerPbf`;
DELIMITER //
CREATE PROCEDURE `LaporanRekapPenerimaanBarangPerPbf`(
	IN `TGLAWAL` DATETIME,
   ...
   SET vKATEGORIBARANG = CONCAT(KATEGORIBARANG,'%');
	...
	SET @sqlText = CONCAT('
			SELECT inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST, py.NAMA NAMAREKANAN, py.ALAMAT ALAMATREKANAN, py.TELEPON TLPREKANAN, pb.FAKTUR NOFAKTUR, pb.TANGGAL TGLFAKTUR, pb.TANGGAL_DIBUAT
				...
				WHERE pb.ID=pbd.PENERIMAAN AND pbd.BARANG=ib.ID AND pb.TANGGAL_DIBUAT BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,'''
