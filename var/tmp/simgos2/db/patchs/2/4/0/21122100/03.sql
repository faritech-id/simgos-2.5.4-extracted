...
BEGIN
	DROP TEMPORARY TABLE IF EXISTS TEMP_KUNJUNGAN;  
     
    CREATE TEMPORARY TABLE TEMP_KUNJUNGAN ENGINE=MEMORY
    SELECT k.NOPEN, k.RUANGAN, k.RUANG_KAMAR_TIDUR
      FROM pendaftaran.kunjungan k
     WHERE k.RUANG_KAMAR_TIDUR > 0
       AND k.`STATUS` = 1
       AND k.KELUAR IS NULL;
    
	 REPLACE INTO informasi.tempat_tidur_kemkes(IDINSTALASI, INSTALASI, IDUNIT, UNIT, IDSUBUNIT, SUBUNIT, IDKAMAR, KAMAR, IDKELAS, KELAS, JENISKAMAR, TTLAKI, TTPEREMPUAN, JMLLAKI, JMLPEREMPUAN, TERPAKAI_KONFIRMASI, LASTUPDATED)
		...
		FROM (
				 SELECT (SELECT i.ID FROM master.ruangan i WHERE e.ID LIKE CONCAT(i.ID,'%')  AND i.JENIS=3 LIMIT 1) IDINSTALASI
						...
						, SUM(IF(dm.ICD IN ('B97.29','U07.1','U07.2'),1,0)) TERPAKAI_KONFIRMASI
					  FROM master.ruang_kamar_tidur a
							 ...
							 LEFT JOIN master.ruangan e ON b.RUANGAN = e.ID AND e.JENIS = 5 AND e.`STATUS` = 1
					  		 LEFT JOIN TEMP_KUNJUNGAN f 
									  ON f.RUANGAN = b.RUANGAN AND f.RUANG_KAMAR_TIDUR = a.ID
					  		 ...
					  		 LEFT JOIN master.diagnosa_masuk dm ON g.DIAGNOSA_MASUK=dm.ID
