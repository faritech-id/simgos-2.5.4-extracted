...
-- Dumping structure for procedure laporan.LaporanRekapLayananFarmasi
DROP PROCEDURE IF EXISTS `LaporanRekapLayananFarmasi`;
DELIMITER //
CREATE PROCEDURE `LaporanRekapLayananFarmasi`(
	IN `TGLAWAL` DATETIME,
   ...
   SET vKATEGORIBARANG = CONCAT(KATEGORIBARANG,'%');
	...
	SET @sqlText = CONCAT('
		SELECT inst.PPK, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST
		   , CONCAT(''LAPORAN REKAP PELAYANAN '',UPPER(jk.DESKRIPSI),'' PER RUANGAN'') JENISLAPORAN
			, rg.DESKRIPSI RUANG, IFNULL(master.getHeaderKategoriBarang(IF(''',KATEGORIBARANG,'''!=''0'',''',KATEGORIBARANG,''',IF(''',KATEGORI,'''!=''0'',''',KATEGORI,''',''',JENISINVENTORY,'''))),''Semua'') KATEGORI
			, SUM(lf.JUMLAH) JMLOBAT, rt.TARIF, (SUM(lf.JUMLAH * rt.TARIF)) TOTAL
