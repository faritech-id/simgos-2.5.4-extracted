...
CREATE TRIGGER `onAfterUpdateFarmasi` AFTER UPDATE ON `farmasi` FOR EACH ROW BEGIN
	...
	/* Script Untuk Insert Ulang Mutasi */
	IF NEW.FLAG != OLD.FLAG AND NEW.FLAG = 1 THEN
		CALL layanan.executeRecalculateMutasiBarang(NEW.KUNJUNGAN, NEW.TANGGAL, NEW.FARMASI, NEW.JUMLAH, NEW.BON, NEW.HARI, NEW.ID, NEW.TINDAKAN_PAKET,1);
	END IF;
	IF NEW.FLAG != OLD.FLAG AND NEW.FLAG = 99 THEN
		CALL layanan.executeRecalculateMutasiBarang(NEW.KUNJUNGAN, NEW.TANGGAL, NEW.FARMASI, NEW.JUMLAH, NEW.BON, NEW.HARI, NEW.ID, NEW.TINDAKAN_PAKET,99);
	END IF;
	/* End Script Untuk Insert Ulang Mutasi */
			...
			IF pembayaran.isFinalTagihan(VTAGIHAN) = 0 THEN
				IF OLD.STATUS = 1 AND NEW.STATUS = 2 THEN
					IF OLD.FARMASI > 0 THEN
						IF OLD.ALASAN_TIDAK_TERLAYANI = "" OR NEW.ALASAN_TIDAK_TERLAYANI = "" OR OLD.ALASAN_TIDAK_TERLAYANI IS NULL THEN
							BEGIN
								SELECT br.ID, IF(k.KELUAR IS NULL, NEW.TANGGAL, k.KELUAR) INTO VBARANG_RUANGAN, VTGLFINALKUNJUNGAN
								  FROM pendaftaran.kunjungan k
								...
								IF FOUND_ROWS() > 0 THEN
									INSERT INTO inventory.transaksi_stok_ruangan(BARANG_RUANGAN, JENIS, REF, TANGGAL, JUMLAH)
										  VALUES(VBARANG_RUANGAN, 33, OLD.ID, VTGLFINALKUNJUNGAN, (NEW.JUMLAH - NEW.BON));
								END IF;
								...
								IF VSTATUS_VALIDASI_HARI > 0 THEN
									SET VHARI = NEW.HARI;
								ELSE
									SET VHARI = IF(NEW.HARI > 0,NEW.HARI,0);
								END IF;
								...
								IF VHARI > 0 THEN
									...
									IF VJML_ROWBTS > 0 THEN
										IF EXISTS(SELECT 1 FROM aplikasi.properti_config WHERE ID = 43 AND VALUE = 'TRUE') THEN
					...
					IF NEW.FARMASI > 0 THEN										
						IF OLD.ALASAN_TIDAK_TERLAYANI = '' OR NEW.ALASAN_TIDAK_TERLAYANI = '' OR OLD.ALASAN_TIDAK_TERLAYANI IS NULL THEN
							BEGIN
								...
								IF FOUND_ROWS() > 0 THEN
									INSERT INTO inventory.transaksi_stok_ruangan(BARANG_RUANGAN, JENIS, REF, TANGGAL, JUMLAH)
										  VALUES(VBARANG_RUANGAN, 35, OLD.ID, VTGLFINALKUNJUNGAN, (OLD.JUMLAH - OLD.BON));
								END IF;
