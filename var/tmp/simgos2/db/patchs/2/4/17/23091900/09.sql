...
-- Dumping structure for procedure laporan.LaporanBarangExpiredDatePerTanggalPenerimaan
DROP PROCEDURE IF EXISTS `LaporanBarangExpiredDatePerTanggalPenerimaan`;
DELIMITER //
CREATE PROCEDURE `LaporanBarangExpiredDatePerTanggalPenerimaan`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
  ...
  SET @sqlText = CONCAT('
    ...
    WHERE pb.ID=pbd.PENERIMAAN AND pbd.BARANG=ib.ID AND pbd.STATUS!=0  AND pb.TANGGAL_PENERIMAAN BETWEEN ''',VTGLAWAL,''' AND ''',VTGLAKHIR,'''
