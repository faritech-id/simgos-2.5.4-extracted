...
BEGIN
	...
	CREATE TEMPORARY TABLE TEMP_HEADER_RINCIAN ENGINE=MEMORY
		SELECT i.PPK, ppk.NAMA NAMAINSTANSI, ppk.ALAMAT ALAMATINSTANSI, t.ID NOMOR_TAGIHAN
				 ...
				 , IF(pt.OLEH=0,pt.DESKRIPSI,master.getNamaLengkapPegawai(mp.NIP)) PENGGUNA, t.ID IDTAGIHAN, (t.TOTAL+ pembayaran.getTarifAmbulance(t.ID)) TOTALRS
				 , w.DESKRIPSI WILAYAH
				...
				,  @tghn:=(IF(pj.JENIS=2 AND pjt.NAIK_KELAS=1,(pjt.TOTAL_NAIK_KELAS), IF(pj.JENIS=2 AND pjt.NAIK_KELAS_VIP=1, pjt.TARIF_INACBG_KELAS1,(t.TOTAL  + pembayaran.getTarifAmbulance(t.ID)))) + IF(pjt.SELISIH_MINIMAL IS NULL,0,pjt.SELISIH_MINIMAL)) TOTALTAGIHAN
				 ...
				 , (IF(pt.TOTAL IS NULL, IF((ROUND(pembayaran.getTotalTagihanPembayaran(t.ID)) + ROUND(t.PEMBULATAN)) < 0 ,0,(ROUND(pembayaran.getTotalTagihanPembayaran(t.ID)) + ROUND(t.PEMBULATAN))), pt.TOTAL) ) + pembayaran.getTarifAmbulance(t.ID)   TOTALJUMLAHBAYAR
			    , (IF(pt.TOTAL IS NULL, IF((ROUND(pembayaran.getTotalTagihanPembayaran(t.ID)) + ROUND(t.PEMBULATAN)) < 0 ,0,(ROUND(pembayaran.getTotalTagihanPembayaran(t.ID)) + ROUND(t.PEMBULATAN))), pt.TOTAL) -@tdp)  + pembayaran.getTarifAmbulance(t.ID) JUMLAHBAYAR
		  ...
		  FROM pembayaran.tagihan t
		       ...
		       LEFT JOIN master.referensi rf ON pj.JENIS=rf.ID AND rf.JENIS=10
		  		 LEFT JOIN pembayaran.pembayaran_tagihan pt ON pt.TAGIHAN = t.ID AND pt.JENIS = 1 AND pt.STATUS=2
	...
	INSERT INTO TEMP_DETIL_RINCIAN
		SELECT rt.TAGIHAN,
				 ...
				 IF(r1.JENIS_KUNJUNGAN=15,'Administrasi', adm.NAMA) LAYANAN,
				 IF(r1.JENIS_KUNJUNGAN=15, p.TANGGAL,IF(rt.JENIS = 1, 
			    ...
			    , '' DOKTERKEDUA
				 ...
				 , IF(r1.JENIS_KUNJUNGAN=15,'Administrasi', adm.NAMA) LAYANAN1
				 , '' LAYANAN_OK
				 , '' TARIF_LAYANAN_OK
				 , '' RP
	...
	SELECT rt.TAGIHAN, 
			 ...
			 rt.JUMLAH, (rt.TARIF + pembayaran.getTarifAmbulance(rt.TAGIHAN)) TARIF
			 , r.JENIS_KUNJUNGAN
