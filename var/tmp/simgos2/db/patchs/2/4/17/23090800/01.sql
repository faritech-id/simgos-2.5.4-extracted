...
BEGIN
	SET @sqlText = CONCAT('
		SELECT r.*, 
			 ...
			 master.getNamaLengkap(p.NORM) NAMALENGKAP, pj.JENIS IDCARABAYAR, kap.NOMOR NOMORKARTU, rf.DESKRIPSI CARABAYAR,
			...
			, IF(INSTR(dm.DIAGNOSA, ''B20'') > 1 OR  INSTR(dm.DIAGNOSA, ''HIV'') > 1,'''',dm.DIAGNOSA) DIAGNOSA
				 ...
				 , pj.NOMOR NOSEP
				 , IFNULL((SELECT SUM(tj.TOTAL) 
							FROM pembayaran.penjamin_tagihan tj 
								, `master`.referensi cr 
							WHERE tj.PENJAMIN=cr.ID AND cr.JENIS=10 AND INSTR(cr.DESKRIPSI,''JASA RAHARJA'') > 0
							AND tj.TAGIHAN=t.ID),0) JAMINAN_JASA_RAHARJA
				 ,	IFNULL((SELECT 1 
					FROM pembayaran.penjamin_tagihan tj 
						, `master`.referensi cr 
					WHERE tj.PENJAMIN=cr.ID AND cr.JENIS=10 AND INSTR(cr.DESKRIPSI,''JASA RAHARJA'') > 0
					AND tj.TAGIHAN=t.ID LIMIT 1),0) STATUS_JASA_RAHARJA
			  ,	IFNULL((SELECT cr.DESKRIPSI 
					FROM pembayaran.penjamin_tagihan tj 
						, `master`.referensi cr 
					WHERE tj.PENJAMIN=cr.ID AND cr.JENIS=10 AND INSTR(cr.DESKRIPSI,''JASA RAHARJA'') > 0
					AND tj.TAGIHAN=t.ID LIMIT 1),'''') CARA_BAYAR_JASA_RAHARJA
