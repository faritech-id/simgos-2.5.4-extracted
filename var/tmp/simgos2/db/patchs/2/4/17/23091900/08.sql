  ...
  SET @sqlText = CONCAT('
    ...
    FROM inventory.penerimaan_barang pb
      ...
      , inventory.barang ib  
        ...
        LEFT JOIN inventory.penggolongan_barang gb ON ib.ID=gb.PENGGOLONGAN
      , (SELECT mp.NAMA, ai.PPK, mp.ALAMAT
            FROM aplikasi.instansi ai
              , master.ppk mp
            WHERE ai.PPK=mp.ID) inst
    WHERE pb.ID=pbd.PENERIMAAN AND pbd.BARANG=ib.ID AND pbd.STATUS!=0  AND pb.MASA_BERLAKU BETWEEN ''',VTGLAWAL,''' AND ''',VTGLAKHIR,'''
