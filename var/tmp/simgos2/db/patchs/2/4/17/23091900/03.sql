...
-- Dumping structure for procedure laporan.LaporanFarmasiPerpasien
DROP PROCEDURE IF EXISTS `LaporanFarmasiPerpasien`;
DELIMITER //
CREATE PROCEDURE `LaporanFarmasiPerpasien`(
	IN `TGLAWAL` DATETIME,
	...
	SET @sqlText = CONCAT('
		SELECT inst.PPK, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST,lf.ID NOMOR, lf.KUNJUNGAN, lf.TANGGAL, master.getNamaLengkapPegawai(mp.NIP) NAMADOKTER
		   , CONCAT(''LAPORAN PELAYANAN '',UPPER(jk.DESKRIPSI),'' PER PASIEN'') JENISLAPORAN
			...
			rt.TARIF, (lf.JUMLAH * rt.TARIF) TOTAL,  CONCAT(lf.RACIKAN,lf.GROUP_RACIKAN) RACIKAN, lf.`STATUS` STATUSLAYANAN
			...
			, master.getDiagnosaPasien(pp.NOMOR) DIAGNOSA, master.getProsedurePasien(pp.NOMOR) TINDAKAN
			, kode.kode KODECBG, kode.DESKRIPSI NAMACBG, cbg.TARIFCBG
			, master.getKodeDiagnosaPasien(pp.NOMOR) KODEICD
			...
			GROUP BY lf.ID
			ORDER BY TANGGAL, RUANG, ASALPENGIRIM, NAMADOKTER	
			');
