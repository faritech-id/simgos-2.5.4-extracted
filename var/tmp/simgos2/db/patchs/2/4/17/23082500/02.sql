...
-- Dumping structure for procedure laporan.LaporanDetailKoreksi
DROP PROCEDURE IF EXISTS `LaporanDetailKoreksi`;
DELIMITER //
CREATE PROCEDURE `LaporanDetailKoreksi`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
   ...
   SET vKATEGORI = CONCAT(KATEGORI,'%');
	...
	SET @sqlText = CONCAT('
		SELECT 
		CONCAT(''DETAIL LAPORAN TRANSAKSI KOREKSI PER ALASAN'') JENISLAPORAN, inst.NAMA NAMAINST, inst.ALAMAT ALAMATINST
		, master.getHeaderLaporan(''',RUANGAN,''') INSTALASI
		, IF(t.JENIS=1,''Masuk'',''Keluar'') JENIS, t.TANGGAL, t.KET, IF(t.JENIS = 1, ra1.DESKRIPSI, ra2.DESKRIPSI) NMALASAN, b.NAMA NMBARANG, d.JUMLAH, t.TANGGAL
		,inventory.getHargaPenerimaanTerakhir(b.ID) HPT, (inventory.getHargaPenerimaanTerakhir(b.ID)* (d.JUMLAH)) HRG_TOT
