...
BEGIN
	...
	SET @sqlText = CONCAT('
		SELECT @PPK PPK, @NAMAINST NAMAINST, @ALAMATINST ALAMATINST, @KOTA KOTA, master.getNamaLengkapPegawai(mp.NIP) KASIR,
			     p.NORM, master.getNamaLengkap(p.NORM) NAMAPASIEN, ps.NAMA NAMAPEMBAYAR,pp.NOMOR NOPEN, pt.TAGIHAN, DATE(pt.TANGGAL) TGLBAYAR,
			     pt.JENIS, crbyr.DESKRIPSI CARABAYAR, pt.TANGGAL, tg.TOTAL TOTALTAGIHAN, p3.TAGIHAN_PIUTANG,
			     IF(''',PTAGIHAN_PIUTANG,'''=0,(SELECT SUM(bpp.TOTAL)
			      ...
			      @JMLPEMBAYARAN:=@JMLPEMBAYARAN + tg.TOTAL JMLPEMBAYARAN
	   FROM  pembayaran.pembayaran_tagihan pt
		...
		WHERE  pt.`STATUS` = 2 AND pt.JENIS IN (1,5)  AND pt.TOTAL > 0
		  AND p.NORM=',PNORM,' 	
