...
-- membuang struktur untuk procedure laporan.LaporanSurveilansRawatInap
DROP PROCEDURE IF EXISTS `LaporanSurveilansRawatInap`;
DELIMITER //
CREATE PROCEDURE `LaporanSurveilansRawatInap`(
	IN `TGLAWAL` DATETIME,
...
BEGIN	
   ...
   SET @sqlText = CONCAT('SELECT COUNT(*) INTO @TOTALKASUS
						...
						WHERE  pk.NOPEN=md.NOPEN AND md.STATUS=1 AND md.INA_GROUPER=0 AND pp.NOMOR=tp.NOPEN AND pk.RUANGAN=tp.RUANGAN
							AND r.JENIS_KUNJUNGAN=3 AND md.KODE=ic.KODE_ICD10 AND md.BARU=1
	...
	SET @sqlText = CONCAT('
		...
		FROM `master`.surveilans_penyakit_menular rl
			  LEFT JOIN (SELECT md.KODE KODEICD10,
								 ...
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=1,1,0)) KLP1,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=2,1,0)) KLP2,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=3,1,0)) KLP3,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=4,1,0)) KLP4,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=5,1,0)) KLP5,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=6,1,0)) KLP6,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=7,1,0)) KLP7,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=8,1,0)) KLP8,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=9,1,0)) KLP9,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=10,1,0)) KLP10,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=11,1,0)) KLP11,
								 SUM(IF(master.getKelompokUmurSurveilans(pp.TANGGAL, ps.TANGGAL_LAHIR)=12,1,0)) KLP12
						FROM pendaftaran.kunjungan pk
							  LEFT JOIN master.ruangan r ON pk.RUANGAN=r.ID AND r.JENIS=5
							  LEFT JOIN layanan.pasien_pulang lpp ON lpp.KUNJUNGAN=pk.NOMOR AND lpp.`STATUS`=1
							  LEFT JOIN master.referensi cr ON lpp.CARA=cr.ID AND cr.JENIS=45
