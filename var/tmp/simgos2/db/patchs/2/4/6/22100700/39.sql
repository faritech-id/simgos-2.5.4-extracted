-- --------------------------------------------------------
-- Host:                         192.168.137.7
-- Server version:               8.0.23 - MySQL Community Server - GPL
-- Server OS:                    Linux
-- HeidiSQL Version:             11.2.0.6213
-- --------------------------------------------------------

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET NAMES utf8 */;
/*!50503 SET NAMES utf8mb4 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;
USE `kemkes-ihs`;
-- Dumping structure for trigger kemkes-ihs.procedure_before_insert
DROP TRIGGER IF EXISTS `procedure_before_insert`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `procedure_before_insert` BEFORE INSERT ON `procedure` FOR EACH ROW BEGIN
	DECLARE VSTATUS CHAR(100);
	DECLARE VCODE, VSUBJECT, VENCOUNTER, VNOTE JSON;
	
	SELECT 
		tcr.code,
		JSON_OBJECT('coding', 
		   JSON_ARRAY(
			 	JSON_OBJECT(
					'system', 'http://hl7.org/fhir/sid/icd-9-cm',
					'code', pro.KODE,
					'display', mrc.STR 	 	 
				)
		   )
		),
		`kemkes-ihs`.getPatient(pen.NORM),
		`kemkes-ihs`.getEncounter(pen.NOMOR),
		JSON_ARRAY(
			JSON_OBJECT(
				'text', pro.TINDAKAN
			)
		)
	INTO
		VSTATUS,
		VCODE,
		VSUBJECT,
		VENCOUNTER,
		VNOTE
	FROM 
		medicalrecord.prosedur pro 
	LEFT JOIN pendaftaran.pendaftaran pen ON pen.NOMOR = pro.NOPEN
	LEFT JOIN `master`.mrconso mrc ON mrc.CODE = pro.KODE
	, `kemkes-ihs`.type_code_reference tcr
	WHERE pro.ID = NEW.refId 
	AND tcr.`type` = 11 
	AND tcr.id = 6 LIMIT 1;
	
	SET NEW.status = VSTATUS;
	SET NEW.code = VCODE;
	SET NEW.subject = VSUBJECT;
	SET NEW.encounter = VENCOUNTER;
	SET NEW.note = VNOTE;
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

/*!40101 SET SQL_MODE=IFNULL(@OLD_SQL_MODE, '') */;
/*!40014 SET FOREIGN_KEY_CHECKS=IFNULL(@OLD_FOREIGN_KEY_CHECKS, 1) */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40111 SET SQL_NOTES=IFNULL(@OLD_SQL_NOTES, 1) */;
