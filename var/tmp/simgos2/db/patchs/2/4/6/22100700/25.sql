...
USE `kemkes-ihs`;
-- Dumping structure for procedure kemkes-ihs.pendaftaranToEncounter
DROP PROCEDURE IF EXISTS `pendaftaranToEncounter`;
DELIMITER //
CREATE PROCEDURE `pendaftaranToEncounter`()
BEGIN
	...
	SELECT DISTINCT p.NOMOR,p.TANGGAL
		...
		AND p.`STATUS` != 0
		AND k.MASUK > s.TANGGAL_TERAKHIR
		ORDER BY p.TANGGAL;
	...
	EOF: LOOP
		...
		IF NOT EXISTS(SELECT 1 FROM `kemkes-ihs`.encounter p WHERE p.refId = VNOMOR) THEN
			INSERT INTO `kemkes-ihs`.encounter(refId)
			   VALUES (VNOMOR);
		END IF;
