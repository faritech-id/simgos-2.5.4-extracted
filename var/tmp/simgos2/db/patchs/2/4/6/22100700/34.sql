...
BEGIN
	...
	DECLARE VID, VSISTOLIK, VDISTOLIK, VFREKUENSI_NADI, VFREKUENSI_NAFAS, VSUHU, VNORM INT;
	...
	OPEN CR_RUANGAN;
	EOF: LOOP
		FETCH CR_RUANGAN INTO VID, VSISTOLIK, VDISTOLIK, VFREKUENSI_NADI, VFREKUENSI_NAFAS, VSUHU, VNOPEN, VTANGGAL, VTGL_INPUT, VNORM;
		...
		/* SET SUBJECT ATAU PASINE DAN ENCOUNTER YANG MELAKUKAN PEMERIKSAAN */
		...
		IF NOT EXISTS(SELECT 1 FROM `kemkes-ihs`.observation r WHERE r.refId = VID AND r.jenis = 1) THEN
			...
			INSERT INTO `kemkes-ihs`.observation(refId, jenis, `status`, valueQuantity, category, nopen, `code`, subject, effectiveDateTime, encounter)
			   VALUES (VID, 1, 'final', JSON_OBJECT(
					'value', VFREKUENSI_NADI,
					'unit', 'beats/minute',
					'system', 'http://unitsofmeasure.org',
					'code', '/min'
				), VCATEGORY, VNOPEN, VCODE, VSUBJECT, VTGL_INPUT, VENCOUNTER);
		END IF;
