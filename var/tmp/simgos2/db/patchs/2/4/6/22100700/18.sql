...
CREATE TRIGGER `encounter_before_update` BEFORE UPDATE ON `encounter` FOR EACH ROW BEGIN
	...
	IF NEW.send = 1 AND OLD.send != NEW.send THEN
		...
		LEFT JOIN medicalrecord.diagnosa pr ON pr.ID = co.refId
		LEFT JOIN `master`.mrconso mrc ON mrc.CODE = pr.KODE
		WHERE co.nopen = OLD.refId AND co.id IS NOT NULL AND pr.`STATUS` = 1;
		
		IF VDIAGNOSTIC IS NOT NULL THEN
			SET VDIAGNOSTIC = JSON_ARRAY(VDIAGNOSTIC);
		END IF;
