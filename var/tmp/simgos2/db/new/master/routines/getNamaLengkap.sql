...
-- membuang struktur untuk function master.getNamaLengkap
DROP FUNCTION IF EXISTS `getNamaLengkap`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `getNamaLengkap`(`PNORM` VARCHAR(8)) RETURNS varchar(75) CHARSET latin1
    DETERMINISTIC
BEGIN
