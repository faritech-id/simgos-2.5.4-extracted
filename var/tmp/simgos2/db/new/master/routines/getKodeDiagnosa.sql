...
-- membuang struktur untuk function master.getKodeDiagnosa
DROP FUNCTION IF EXISTS `getKodeDiagnosa`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `getKodeDiagnosa`(`PNOPEN` CHAR(10), `PUTAMA` INT) RETURNS text CHARSET latin1
    DETERMINISTIC
BEGIN
	DECLARE HASIL TEXT;
