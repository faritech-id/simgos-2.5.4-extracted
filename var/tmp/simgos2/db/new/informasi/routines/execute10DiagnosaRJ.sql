...
-- membuang struktur untuk procedure informasi.execute10DiagnosaRJ
DROP PROCEDURE IF EXISTS `execute10DiagnosaRJ`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `execute10DiagnosaRJ`(IN `PTGL_AWAL` DATE, IN `PTGL_AKHIR` DATE)
BEGIN
	...
	BEGIN		
		...
		DECLARE CR_EXEC CURSOR FOR SELECT DATE(tk.MASUK) TANGGAL
				...
				WHERE pd.NOMOR=tk.NOPEN AND pd.STATUS IN (1,2) AND tk.STATUS IN (1,2) AND tk.REF IS NULL
						AND tk.RUANGAN=su.ID AND su.JENIS=5 AND su.JENIS_KUNJUNGAN=1 AND tk.NOPEN=md.NOPEN AND md.STATUS=1
		...
		EXIT_EXEC: LOOP
			...
			IF EXISTS(SELECT 1 FROM informasi.diagnosa_rj WHERE TANGGAL = VTANGGAL AND ID = VID) THEN
				UPDATE informasi.diagnosa_rj
				   SET VALUE = VVALUE, LASTUPDATED = VLASTUPDATED
				  WHERE TANGGAL = VTANGGAL AND ID = VID;
			ELSE
				INSERT INTO informasi.diagnosa_rj(TANGGAL, ID, DESKRIPSI, VALUE, LASTUPDATED)
