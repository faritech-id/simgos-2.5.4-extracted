...
-- membuang struktur untuk procedure informasi.list10DiagnosaRJ
DROP PROCEDURE IF EXISTS `list10DiagnosaRJ`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `list10DiagnosaRJ`(IN `PTGL_AWAL` DATE, IN `PTGL_AKHIR` DATE, IN `PTYPE` TINYINT)
BEGIN
	...
	IF PTYPE < 10 THEN
		SET @sqlText = CONCAT('
			SELECT  YEAR(TANGGAL) TAHUN, MONTH(TANGGAL) IDBULAN, b.NAMA BULAN, r.ID, DESKRIPSI, SUM(VALUE) VALUE , MAX(LASTUPDATED) LASTUPDATED
			  FROM informasi.diagnosa_rj r LEFT JOIN master.bulan b ON MONTH(TANGGAL) = b.ID
