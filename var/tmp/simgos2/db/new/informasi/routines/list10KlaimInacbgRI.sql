...
-- membuang struktur untuk procedure informasi.list10KlaimInacbgRI
DROP PROCEDURE IF EXISTS `list10KlaimInacbgRI`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `list10KlaimInacbgRI`(IN `PTGL_AWAL` DATE, IN `PTGL_AKHIR` DATE, IN `PTYPE` TINYINT)
BEGIN
	...
	IF PTYPE < 10 THEN
		SET @sqlText = CONCAT('
			SELECT  YEAR(TANGGAL) TAHUN, MONTH(TANGGAL) IDBULAN, b.NAMA BULAN, r.ID, DESKRIPSI, SUM(VALUE) VALUE , MAX(LASTUPDATED) LASTUPDATED
			  FROM informasi.klaim_inacbg_ri r LEFT JOIN master.bulan b ON MONTH(TANGGAL) = b.ID
