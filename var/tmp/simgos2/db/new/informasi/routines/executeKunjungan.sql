...
-- membuang struktur untuk procedure informasi.executeKunjungan
DROP PROCEDURE IF EXISTS `executeKunjungan`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `executeKunjungan`(IN `PTGL_AWAL` DATE, IN `PTGL_AKHIR` DATE
...
BEGIN
	...
	BEGIN		
		...
		EXIT_EXEC: LOOP
			...
			ELSE
				INSERT INTO informasi.kunjungan(TANGGAL, ID, DESKRIPSI, IDINSTALASI, INSTALASI, IDUNIT, UNIT, IDSUBUNIT, SUBUNIT, CARABAYAR, IDCARABAYAR, VALUE, LASTUPDATED)
