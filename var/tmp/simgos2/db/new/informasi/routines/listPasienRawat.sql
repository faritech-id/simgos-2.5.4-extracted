...
-- membuang struktur untuk procedure informasi.listPasienRawat
DROP PROCEDURE IF EXISTS `listPasienRawat`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `listPasienRawat`(IN `PTGL_AWAL` DATE, IN `PTGL_AKHIR` DATE, IN `PTYPE` TINYINT)
BEGIN	
	...
	IF PTYPE < 10 THEN
		SET @sqlText = CONCAT('
			SELECT YEAR(TANGGAL) TAHUN, MONTH(TANGGAL) IDBULAN, b.NAMA BULAN, p.ID, DESKRIPSI, CARABAYAR, IDCARABAYAR, SUM(VALUE) VALUE, MAX(LASTUPDATED) LASTUPDATED
			 ...
			 WHERE TANGGAL BETWEEN ''', PTGL_AWAL, ''' AND ''', PTGL_AKHIR, ''' AND p.ID = 2 ',
