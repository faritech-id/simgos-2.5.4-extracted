...
-- membuang struktur untuk procedure layanan.storeOrderRadDiTindakan
DROP PROCEDURE IF EXISTS `storeOrderRadDiTindakan`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `storeOrderRadDiTindakan`(IN `PID` CHAR(21), IN `POLEH` SMALLINT)
BEGIN
	...
	DECLARE CRDATA CURSOR FOR
		SELECT c.NOMOR, b.TINDAKAN
		  FROM layanan.order_rad a,
		  	 	 layanan.order_detil_rad b,
	...
	OPEN CRDATA;
		DATA_END: LOOP
			...
			BEGIN
				...
				UPDATE layanan.order_detil_rad
				   SET REF = VIDTINDAKAN
				 WHERE ORDER_ID = PID
