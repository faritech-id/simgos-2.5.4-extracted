...
-- membuang struktur untuk procedure laporan.LaporanKegiatanRawatInap
DROP PROCEDURE IF EXISTS `LaporanKegiatanRawatInap`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanKegiatanRawatInap`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
  ...
  FROM master.ruangan mr
		 LEFT JOIN (
				   	...
				   	GROUP BY tp.RUANGAN
						...
						GROUP BY pk.RUANGAN
						) b ON b.RUANGAN LIKE CONCAT(mr.ID,''%'')
	, (SELECT p.NAMA NAMAINST, p.ALAMAT ALAMATINST
				FROM aplikasi.instansi ai
					, master.ppk p
				WHERE ai.PPK=p.ID) INST
	WHERE mr.ID LIKE ''',vRUANGAN,''' AND mr.JENIS_KUNJUNGAN=3 AND mr.JENIS_KUNJUNGAN=''',LAPORAN,'''
	GROUP BY mr.ID
	');
