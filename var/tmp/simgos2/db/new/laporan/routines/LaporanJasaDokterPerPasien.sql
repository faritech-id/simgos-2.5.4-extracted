...
-- membuang struktur untuk procedure laporan.LaporanJasaDokterPerPasien
DROP PROCEDURE IF EXISTS `LaporanJasaDokterPerPasien`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaDokterPerPasien`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
   ...
   SET vRUANGAN = CONCAT(RUANGAN,'%');
	...
	SET @sqlText = CONCAT(
	  'SELECT INST.NAMAINST, INST.ALAMATINST, NOPEN,UNITPELAYANAN, NORM, NAMAPASIEN, TANGGALREGISTRASI, TANGGALTINDAKAN, NAMATINDAKAN, JUMLAH
					 , CARABAYAR, MEDIS, PETUGASMEDIS, '''' DOKANASTESI, master.getHeaderLaporan(''',RUANGAN,''') INSTALASI
			 ...
			 FROM (
					...
					SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
							...
							WHERE pt.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND pt.`STATUS` !=0 AND pt.JENIS=1
							  AND rt.TAGIHAN=pt.TAGIHAN AND  rt.`STATUS` NOT IN (0) AND mtt.DOKTER_OPERATOR !=0
							  ',IF(RUANGAN=0,'',CONCAT(' AND kjgn.RUANGAN LIKE ''',vRUANGAN,'''')),' 
								',IF(CARABAYAR=0,'',CONCAT(' AND pj.JENIS=',CARABAYAR)),'
								',IF(DOKTER=0,'',CONCAT(' AND ptm.MEDIS=',DOKTER)),'
							GROUP BY ptm.MEDIS, kjgn.RUANGAN, kjgn.NOPEN, tm.TINDAKAN,tm.TANGGAL
