...
-- membuang struktur untuk procedure laporan.LaporanJasaPenataPerPasienPalu
DROP PROCEDURE IF EXISTS `LaporanJasaPenataPerPasienPalu`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaPenataPerPasienPalu`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
	  ...
	  'SELECT * FROM (
	  	SELECT RAND() QID, INST.NAMAINST, INST.ALAMATINST,kjgn.NOPEN, DATE_FORMAT(tm.TANGGAL,''%d-%m-%Y %H:%i:%s'') TANGGALTINDAKAN, tm.TANGGAL
						   ...
						   , crbyr.DESKRIPSI CARABAYAR, ptm.MEDIS
							, master.getNamaLengkapPegawai(mpper.NIP) PETUGASMEDIS
			           	, 0 DISKON
			           	, (IF(pj.JENIS=2
			           			/* berlakukan aturan untuk pasien BPJS*/
									,IF(tr.ID IS NOT NULL,((0.2 * hslgr.TOTALTARIF)*0.3), (0.18 * hslgr.TOTALTARIF))
						  			,mtt.PENATA_ANASTESI
						  			)) DOKTER_OPERATOR
				FROM pembayaran.pembayaran_tagihan pt
				   , pembayaran.rincian_tagihan rt
				     ...
				     LEFT JOIN master.tindakan_keperawatan tk ON tm.TINDAKAN=tk.TINDAKAN AND tk.`STATUS`=1
				  	  LEFT JOIN layanan.petugas_tindakan_medis ptm ON tm.ID=ptm.TINDAKAN_MEDIS AND ptm.JENIS=7  AND ptm.KE=1 AND ptm.STATUS=1
				  	  LEFT JOIN master.perawat per ON ptm.MEDIS=per.ID
					  LEFT JOIN master.pegawai mpper ON per.NIP=mpper.NIP
