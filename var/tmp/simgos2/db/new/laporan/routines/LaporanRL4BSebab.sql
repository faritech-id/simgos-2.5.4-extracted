...
-- membuang struktur untuk procedure laporan.LaporanRL4BSebab
DROP PROCEDURE IF EXISTS `LaporanRL4BSebab`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRL4BSebab`(
	IN `TGLAWAL` DATETIME,
	...
	SET @sqlText = CONCAT('
		...
		FROM master.jenis_laporan_detil jrl
			...
			WHERE  pk.NOPEN=md.NOPEN AND md.STATUS=1 AND pp.NOMOR=tp.NOPEN AND pk.RUANGAN=tp.RUANGAN
				AND r.JENIS_KUNJUNGAN IN (1,2) AND md.KODE=ic.KODE AND ic.IDJENISRL=2
