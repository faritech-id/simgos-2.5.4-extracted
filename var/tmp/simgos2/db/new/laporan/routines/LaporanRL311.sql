...
-- membuang struktur untuk procedure laporan.LaporanRL311
DROP PROCEDURE IF EXISTS `LaporanRL311`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRL311`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME
)
BEGIN	
   ...
   SET TAHUN = DATE_FORMAT(TGLAKHIR,'%Y');
	...
	SET @sqlText = CONCAT('
		SELECT INST.*, rl.KODE, rl.DESKRIPSI, ''',TAHUN,''' TAHUN, SUM(IF(rl311.JUMLAH IS NULL,0,rl311.JUMLAH)) JUMLAH
