...
-- membuang struktur untuk procedure laporan.LaporanKunjunganPerCaraBayar
DROP PROCEDURE IF EXISTS `LaporanKunjunganPerCaraBayar`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanKunjunganPerCaraBayar`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
	...
	SET @sqlText = CONCAT('
	SELECT CONCAT(''LAPORAN KUNJUNGAN '',UPPER(jk.DESKRIPSI), '' PER CARA BAYAR'') JENISLAPORAN
			, ref.ID IDCARABAYAR
		...
		WHERE p.NORM=pd.NORM AND pd.NOMOR=tk.NOPEN AND pd.STATUS IN (1,2) AND tk.STATUS IN (1,2)
				AND tk.RUANGAN=jkr.ID AND jkr.JENIS=5 AND jkr.JENIS_KUNJUNGAN=',LAPORAN,'
