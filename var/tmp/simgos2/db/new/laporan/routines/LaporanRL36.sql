...
-- membuang struktur untuk procedure laporan.LaporanRL36
DROP PROCEDURE IF EXISTS `LaporanRL36`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRL36`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME
)
BEGIN	
   ...
   SET TAHUN = DATE_FORMAT(TGLAKHIR,'%Y');
	...
	SET @sqlText = CONCAT('
					SELECT INST.*, ''',TAHUN,''' TAHUN, rl.KODE, rl.DESKRIPSI, SUM(JML) JML
