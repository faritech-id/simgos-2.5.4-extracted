...
-- membuang struktur untuk procedure laporan.SensusHarianPasienMasuk
DROP PROCEDURE IF EXISTS `SensusHarianPasienMasuk`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `SensusHarianPasienMasuk`(IN `TGLAWAL` DATETIME, IN `RUANGAN` CHAR(10))
BEGIN
 ...
 SET @sqlText = CONCAT('
	SELECT INSERT(INSERT(LPAD(p.NORM,6,''0''),3,0,''-''),6,0,''-'') NORM, CONCAT(master.getNamaLengkap(p.NORM)) NAMALENGKAP, IF(p.JENIS_KELAMIN=1,''L'',''P'') JENISKELAMIN
				, pd.NOMOR NOPEN, ref.DESKRIPSI CARABAYAR, CONCAT(rk.KAMAR,'' / '',kelas.DESKRIPSI) KAMAR
			...
			WHERE p.NORM=pd.NORM AND pd.NOMOR=tp.NOPEN  AND pd.NOMOR=tk.NOPEN AND tp.RUANGAN=tk.RUANGAN AND pd.STATUS IN (1,2) AND tk.REF IS NULL
					...
					AND jkr.JENIS_KUNJUNGAN=3 AND tp.RUANGAN LIKE ''',vRUANGAN,''' AND INSTR(rkt.TEMPAT_TIDUR,''Bayi'')=0
