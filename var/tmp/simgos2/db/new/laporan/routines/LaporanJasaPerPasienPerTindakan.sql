...
-- membuang struktur untuk procedure laporan.LaporanJasaPerPasienPerTindakan
DROP PROCEDURE IF EXISTS `LaporanJasaPerPasienPerTindakan`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaPerPasienPerTindakan`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
...
SET @sqlText = CONCAT(
	  'SELECT INST.NAMAINST, INST.ALAMATINST, NOPEN,UNITPELAYANAN, NORM, NAMAPASIEN, TANGGALREGISTRASI, TANGGALTINDAKAN, NAMATINDAKAN, JUMLAH
			 ...
			 FROM (
			 /*tindakan*/
						  SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN
							...
							FROM pembayaran.pembayaran_tagihan pt
							   , pembayaran.rincian_tagihan rt
							     ...
							     LEFT JOIN `master`.tindakan t ON t.ID = tm.TINDAKAN
							  	  LEFT JOIN layanan.petugas_tindakan_medis ptm ON tm.ID=ptm.TINDAKAN_MEDIS AND ptm.JENIS=1 AND ptm.KE=1
							...
							GROUP BY kjgn.NOPEN, tm.TINDAKAN, ptm.MEDIS
					/*Paket*/
							UNION
					...
					SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN
									   ...
									   , master.getNamaLengkap(ps.NORM) NAMAPASIEN
										, DATE_FORMAT(p.TANGGAL,''%d-%m-%Y'') TANGGALREGISTRASI
										, DATE_FORMAT(tm.TANGGAL,''%d-%m-%Y %H:%i:%s'') TANGGALTINDAKAN, tm.TINDAKAN IDTINDAKAN
										, CONCAT(t.NAMA,'' - ['',pkt.NAMA,'']'') NAMATINDAKAN, COUNT(tm.TINDAKAN) JUMLAH
										, crbyr.DESKRIPSI CARABAYAR, ptm.MEDIS
										, master.getNamaLengkapPegawai(mpdok.NIP) PETUGASMEDIS, master.getNamaLengkapPegawai(mpdokanas.NIP) DOKANASTESI
										, SUM(mtt.ADMINISTRASI) ADMINISTRASI, SUM(mtt.SARANA) SARANA, SUM(mtt.BHP) BHP
										, SUM(mtt.DOKTER_OPERATOR) DOKTER_OPERATOR, SUM(mtt.DOKTER_ANASTESI) DOKTER_ANASTESI, SUM(mtt.DOKTER_LAINNYA) DOKTER_LAINNYA
