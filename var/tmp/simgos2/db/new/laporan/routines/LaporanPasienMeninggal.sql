...
-- membuang struktur untuk procedure laporan.LaporanPasienMeninggal
DROP PROCEDURE IF EXISTS `LaporanPasienMeninggal`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanPasienMeninggal`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
