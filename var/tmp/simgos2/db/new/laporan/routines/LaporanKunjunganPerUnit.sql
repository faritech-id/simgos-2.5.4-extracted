...
-- membuang struktur untuk procedure laporan.LaporanKunjunganPerUnit
DROP PROCEDURE IF EXISTS `LaporanKunjunganPerUnit`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanKunjunganPerUnit`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
	...
	SELECT su.DESKRIPSI SUBUNIT
			...
			, INST.NAMAINST, INST.ALAMATINST, CONCAT(''LAPORAN KUNJUNGAN '',UPPER(jk.DESKRIPSI),'' PER UNIT'') JENISLAPORAN
