...
-- membuang struktur untuk procedure laporan.LaporanRP1
DROP PROCEDURE IF EXISTS `LaporanRP1`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRP1`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
