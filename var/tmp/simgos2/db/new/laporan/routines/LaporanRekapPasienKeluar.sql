...
-- membuang struktur untuk procedure laporan.LaporanRekapPasienKeluar
DROP PROCEDURE IF EXISTS `LaporanRekapPasienKeluar`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRekapPasienKeluar`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
 ...
 SET @sqlText = CONCAT('
	SELECT INST.NAMAINST, INST.ALAMATINST, CONCAT(''LAPORAN REKAP PASIEN KELUAR '',UPPER(jk.DESKRIPSI)) JENISLAPORAN,
