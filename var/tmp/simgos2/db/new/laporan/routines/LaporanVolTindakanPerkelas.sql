...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanVolTindakanPerkelas`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `TINDAKAN` INT, IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
