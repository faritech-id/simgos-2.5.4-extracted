...
-- membuang struktur untuk procedure laporan.LaporanJasaPerPasien
DROP PROCEDURE IF EXISTS `LaporanJasaPerPasien`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaPerPasien`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
