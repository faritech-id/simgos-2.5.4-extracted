...
-- membuang struktur untuk procedure laporan.LaporanDistribusiPendapatanPerPasien
DROP PROCEDURE IF EXISTS `LaporanDistribusiPendapatanPerPasien`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanDistribusiPendapatanPerPasien`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
