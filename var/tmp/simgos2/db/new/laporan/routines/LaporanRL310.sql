...
-- membuang struktur untuk procedure laporan.LaporanRL310
DROP PROCEDURE IF EXISTS `LaporanRL310`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRL310`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME
)
BEGIN	
   ...
   SET TAHUN = DATE_FORMAT(TGLAKHIR,'%Y');
	...
	SET @sqlText = CONCAT('
		SELECT INST.*, rl.KODE, rl.DESKRIPSI, ''',TAHUN,''' TAHUN, SUM(IF(rl310.JUMLAH IS NULL,0,rl310.JUMLAH)) JUMLAH
