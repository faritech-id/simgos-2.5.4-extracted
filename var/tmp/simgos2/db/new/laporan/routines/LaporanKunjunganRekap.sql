...
-- membuang struktur untuk procedure laporan.LaporanKunjunganRekap
DROP PROCEDURE IF EXISTS `LaporanKunjunganRekap`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanKunjunganRekap`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
	...
	SET @sqlText = CONCAT('
	SELECT ref.*, a.*, INST.NAMAINST, INST.ALAMATINST, CONCAT(''LAPORAN REKAP KUNJUNGAN '',UPPER(jk.DESKRIPSI)) JENISLAPORAN
