...
-- membuang struktur untuk procedure laporan.TesKolomDinamis
DROP PROCEDURE IF EXISTS `TesKolomDinamis`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `TesKolomDinamis`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN	
   ...
   SELECT GROUP_CONCAT(
			CONCAT('SUM(IF(jkr.ID=''',jkr.ID,''',1,0)) ',REPLACE(UPPER(jkr.DESKRIPSI),' ',''),'
			')) INTO @sql
	FROM master.ruangan jkr
	WHERE jkr.JENIS_KUNJUNGAN=LAPORAN AND jkr.JENIS=5 AND jkr.`STATUS`=1;
  	
 	SET @sqlText = CONCAT('
	SELECT tgl.*, INST.*, a.*, CONCAT(IF(jk.ID=1,''Laporan Pengunjung '', IF(jk.ID=2,''Laporan Kunjungan '',IF(jk.ID=3,''Laporan Pasien Masuk '',''''))), CONCAT(jk.DESKRIPSI,'' Per Hari'')) JENISLAPORAN
			  ...
			  LEFT JOIN (
								...
								FROM master.pasien p
										, pendaftaran.pendaftaran pd
										  ...
										  LEFT JOIN pendaftaran.surat_rujukan_pasien srp ON pd.RUJUKAN=srp.NOMOR
									...
									WHERE p.NORM=pd.NORM AND pd.NOMOR=tp.NOPEN AND pd.NOMOR=tk.NOPEN AND tp.RUANGAN=tk.RUANGAN AND pd.STATUS IN (1,2) AND tk.REF IS NULL
											AND tk.RUANGAN=jkr.ID AND jkr.JENIS=5 AND tk.MASUK BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,'''
