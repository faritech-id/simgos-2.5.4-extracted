...
-- membuang struktur untuk procedure laporan.LaporanJasaDokterPerPasienPalu
DROP PROCEDURE IF EXISTS `LaporanJasaDokterPerPasienPalu`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaDokterPerPasienPalu`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
	  	...
	  	SELECT RAND() QID, INST.NAMAINST, INST.ALAMATINST,kjgn.NOPEN, DATE_FORMAT(tm.TANGGAL,''%d-%m-%Y %H:%i:%s'') TANGGALTINDAKAN, tm.TANGGAL
			           	...
			           	, (IF(pj.JENIS=2
			           			/* berlakukan aturan untuk pasien BPJS*/
									,IF(tr.ID IS NOT NULL,(0.2 * hslgr.TOTALTARIF), (0.18 * hslgr.TOTALTARIF))
						  			,mtt.DOKTER_OPERATOR
						  			)) DOKTER_OPERATOR
