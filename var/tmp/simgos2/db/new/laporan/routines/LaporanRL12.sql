...
-- membuang struktur untuk procedure laporan.LaporanRL12
DROP PROCEDURE IF EXISTS `LaporanRL12`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRL12`(
	IN `TGLAWAL` DATETIME,
...
BEGIN	
	...
	SET @sqlText = CONCAT('
		SELECT INST.*, ''',TAHUN,''' TAHUN, SUM(AWAL) AWAL, SUM(MASUK) MASUK
				...
				, IF(SUM(HP)=0 OR @JMLTT=0,0,ROUND((SUM(HP) * 100) / (@JMLTT * (DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1)),2)) BOR
				, IF(SUM(HPLK)=0 OR @JMLTT=0,0,ROUND((SUM(HPLK) * 100) / (@JMLTT * (DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1)),2)) BORLK
				, IF(SUM(HPPR)=0 OR @JMLTT=0,0,ROUND((SUM(HPPR) * 100) / (@JMLTT * (DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1)),2)) BORPR
				...
				, IF(SUM(DIPINDAHKAN+HIDUP+MATIKURANG48+MATILEBIH48)=0, 0 ,ROUND(((@JMLTT * (DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1)) - SUM(HP)) / (SUM(DIPINDAHKAN+HIDUP+MATIKURANG48+MATILEBIH48)),2)) TOI
				, IF(SUM(DIPINDAHKANLK+HIDUPLK+MATIKURANG48LK+MATILEBIH48LK)=0, 0 ,ROUND(((@JMLTT * (DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1)) - SUM(HPLK)) / (SUM(DIPINDAHKANLK+HIDUPLK+MATIKURANG48LK+MATILEBIH48LK)),2)) TOILK
				, IF(SUM(DIPINDAHKANPR+HIDUPPR+MATIKURANG48PR+MATILEBIH48PR)=0, 0 ,ROUND(((@JMLTT * (DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1)) - SUM(HPPR)) / (SUM(DIPINDAHKANPR+HIDUPPR+MATIKURANG48PR+MATILEBIH48PR)),2)) TOIPR
				...
				, (SELECT COUNT(pd.NOMOR) JUMLAH
						...
						WHERE pd.NOMOR=tk.NOPEN  AND tk.RUANGAN=jkr.ID AND jkr.JENIS=5 AND jkr.JENIS_KUNJUNGAN IN (1,2)
								AND tk.MASUK BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND pd.STATUS IN (1,2) AND tk.STATUS IN (1,2))/(DATEDIFF(''',TGLAKHIR,''',''',TGLAWAL,''')+1) JMLKUNJUNGAN
