...
-- membuang struktur untuk procedure laporan.LaporanJasaRekapAnastesi
DROP PROCEDURE IF EXISTS `LaporanJasaRekapAnastesi`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaRekapAnastesi`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
   ...
   SET vRUANGAN = CONCAT(RUANGAN,'%');
	...
	SET @sqlText = CONCAT(
	  'SELECT INST.NAMAINST, INST.ALAMATINST, NOPEN,UNITPELAYANAN, NORM, NAMAPASIEN, TANGGALREGISTRASI, TANGGALTINDAKAN, NAMATINDAKAN, JUMLAH
					 , CARABAYAR, MEDIS, DOKANASTESI, master.getHeaderLaporan(''',RUANGAN,''') INSTALASI
					 , SUM(UMUM) UMUM
				  	 , SUM(JKN) JKN
					 , SUM(INHEALTH) INHEALTH
					 , SUM(JKD) JKD
					 , SUM(IKS) IKS
					 ...
					 , SUM(TARIF) TARIF, SUM(DISKON) DISKON
			 ...
			 FROM (
						  SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
									   ...
									   , master.getNamaLengkap(ps.NORM) NAMAPASIEN
										...
										, SUM(IF(pj.JENIS=1,mtt.DOKTER_ANASTESI,0)) UMUM
										, SUM(IF(pj.JENIS=2,mtt.DOKTER_ANASTESI,0)) JKN
										, SUM(IF(pj.JENIS=3,mtt.DOKTER_ANASTESI,0)) INHEALTH
										, SUM(IF(pj.JENIS=4,mtt.DOKTER_ANASTESI,0)) JKD
										, SUM(IF(pj.JENIS NOT IN (1,2,3,4),mtt.DOKTER_ANASTESI,0)) IKS
							...
							WHERE pt.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND pt.`STATUS` !=0 AND pt.JENIS=1
							  AND rt.JENIS = 3 AND rt.TAGIHAN=pt.TAGIHAN AND rt.`STATUS` !=0 AND mtt.DOKTER_ANASTESI !=0
							  ',IF(RUANGAN=0,'',CONCAT(' AND kjgn.RUANGAN LIKE ''',vRUANGAN,'''')),' 
								',IF(CARABAYAR=0,'',CONCAT(' AND pj.JENIS=',CARABAYAR)),'
								',IF(DOKTER=0,'',CONCAT(' AND ptmanas.MEDIS=',DOKTER)),'
							GROUP BY ptmanas.MEDIS
							UNION
					/*Paket*/
					SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
										, INSERT(INSERT(INSERT(LPAD(ps.NORM,8,''0''),3,0,''-''),6,0,''-''),9,0,''-'') NORM
							...
							WHERE pt.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND pt.`STATUS` !=0 AND pt.JENIS=1
							  AND rt.TAGIHAN=pt.TAGIHAN AND rt.`STATUS` !=0 AND mtt.DOKTER_ANASTESI !=0
							  ',IF(RUANGAN=0,'',CONCAT(' AND kjgn.RUANGAN LIKE ''',vRUANGAN,'''')),' 
								',IF(CARABAYAR=0,'',CONCAT(' AND pj.JENIS=',CARABAYAR)),'
								',IF(DOKTER=0,'',CONCAT(' AND ptmanas.MEDIS=',DOKTER)),'
							GROUP BY ptmanas.MEDIS
