...
-- membuang struktur untuk procedure laporan.LaporanLamaDirawat
DROP PROCEDURE IF EXISTS `LaporanLamaDirawat`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanLamaDirawat`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
 ...
 SET @sqlText = CONCAT('
	SELECT INST.NAMAINST, INST.ALAMATINST, CONCAT(''LAPORAN LAMA DIRAWAT ('',UPPER(jk.DESKRIPSI),'')'') JENISLAPORAN,
		    ...
		    pk.MASUK TGLMASUK, pk.KELUAR TGLKELUAR ,r.DESKRIPSI UNIT, CONCAT(rk.KAMAR,'' / '',kelas.DESKRIPSI) KAMAR, SUM(DATEDIFF(pk.KELUAR, pk.MASUK)) LD
