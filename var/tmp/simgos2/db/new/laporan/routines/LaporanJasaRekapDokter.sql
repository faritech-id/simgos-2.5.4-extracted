...
-- membuang struktur untuk procedure laporan.LaporanJasaRekapDokter
DROP PROCEDURE IF EXISTS `LaporanJasaRekapDokter`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaRekapDokter`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
			 ...
			 FROM (
						  SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
									   ...
									   , master.getNamaLengkap(ps.NORM) NAMAPASIEN
										...
										, SUM(IF(pj.JENIS=1,mtt.DOKTER_OPERATOR,0)) UMUM
										, SUM(IF(pj.JENIS=2,mtt.DOKTER_OPERATOR,0)) JKN
										, SUM(IF(pj.JENIS=3,mtt.DOKTER_OPERATOR,0)) INHEALTH
										, SUM(IF(pj.JENIS=4,mtt.DOKTER_OPERATOR,0)) JKD
										, SUM(IF(pj.JENIS NOT IN (1,2,3,4),mtt.DOKTER_OPERATOR,0)) IKS
							...
							WHERE pt.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND pt.`STATUS` !=0 AND pt.JENIS=1
							  AND rt.JENIS = 3 AND rt.TAGIHAN=pt.TAGIHAN AND rt.`STATUS` !=0 AND mtt.DOKTER_OPERATOR !=0
					...
					/*Paket*/
					 SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
							...
							WHERE pt.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND pt.`STATUS` !=0 AND pt.JENIS=1
							  AND rt.TAGIHAN=pt.TAGIHAN AND rt.`STATUS` !=0 AND mtt.DOKTER_OPERATOR !=0
							  ',IF(RUANGAN=0,'',CONCAT(' AND kjgn.RUANGAN LIKE ''',vRUANGAN,'''')),' 
								',IF(CARABAYAR=0,'',CONCAT(' AND pj.JENIS=',CARABAYAR)),'
								',IF(DOKTER=0,'',CONCAT(' AND ptm.MEDIS=',DOKTER)),'
							GROUP BY ptm.MEDIS
