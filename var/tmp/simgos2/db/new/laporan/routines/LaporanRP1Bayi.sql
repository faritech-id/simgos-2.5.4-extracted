...
-- membuang struktur untuk procedure laporan.LaporanRP1Bayi
DROP PROCEDURE IF EXISTS `LaporanRP1Bayi`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRP1Bayi`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
  ...
  FROM master.tanggal mt
		 LEFT JOIN (
						 ...
						 WHERE pk.RUANGAN=r.ID AND r.JENIS_KUNJUNGAN=3 AND pk.`STATUS` IN (1,2) AND pk.NOPEN=pp.NOMOR
						   AND pk.RUANGAN LIKE ''',vRUANGAN,''' AND r.JENIS_KUNJUNGAN=''',LAPORAN,''' AND INSTR(rkt.TEMPAT_TIDUR,''Bayi'')!=0
				   	...
				   	GROUP BY bts.TGL
						...
						SELECT RAND() IDX, pk.RUANGAN, bts.TGL
						     , 0 AWAL
							  ...
							  , SUM(IF(rk.KELAS=5,1,0)) VVIP
							  , SUM(IF(rk.KELAS=4,1,0)) VIP
							  , SUM(IF(rk.KELAS=3,1,0)) KLSI
							  , SUM(IF(rk.KELAS=2,1,0)) KLSII
							  , SUM(IF(rk.KELAS=1,1,0)) KLSIII
							  , SUM(IF(rk.KELAS IN (0,6,7,8),1,0)) KLSKHUSUS
							  , 0 HP
						  FROM pendaftaran.kunjungan pk
						       LEFT JOIN master.ruang_kamar_tidur rkt ON pk.RUANG_KAMAR_TIDUR=rkt.ID
