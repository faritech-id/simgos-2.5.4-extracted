...
-- membuang struktur untuk procedure laporan.LaporanRekapPasienMeninggal
DROP PROCEDURE IF EXISTS `LaporanRekapPasienMeninggal`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRekapPasienMeninggal`(
	IN `TGLAWAL` DATETIME,
...
BEGIN
 ...
 SET @sqlText = CONCAT('
	SELECT INST.NAMAINST, INST.ALAMATINST, CONCAT(''LAPORAN REKAP PASIEN MENINGGAL '',UPPER(jk.DESKRIPSI)) JENISLAPORAN,
			 ...
			 r.DESKRIPSI UNIT, COUNT(lpp.KUNJUNGAN) JUMLAH, 
			 SUM(IF(crbyr.ID=1,1,0)) UMUM,
		    SUM(IF(crbyr.ID=2,1,0)) JKN,
		    SUM(IF(crbyr.ID=3,0,0)) INHEALTH,
		    SUM(IF(crbyr.ID=4,0,0)) JKD,
		    SUM(IF(iks.ID IS NOT NULL,1,0)) IKS,
		    SUM(IF(ps.JENIS_KELAMIN=1,1,0)) LAKILAKI,
		    SUM(IF(ps.JENIS_KELAMIN=2,1,0)) PEREMPUAN
