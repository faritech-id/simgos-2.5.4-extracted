...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanFarmasiPerpasien`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT, IN `KATEGORI` INT, IN `BARANG` INT)
BEGIN
