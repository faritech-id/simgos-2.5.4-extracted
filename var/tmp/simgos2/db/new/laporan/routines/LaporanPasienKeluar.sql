...
-- membuang struktur untuk procedure laporan.LaporanPasienKeluar
DROP PROCEDURE IF EXISTS `LaporanPasienKeluar`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanPasienKeluar`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
