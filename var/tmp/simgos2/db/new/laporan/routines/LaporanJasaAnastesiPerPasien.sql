...
-- membuang struktur untuk procedure laporan.LaporanJasaAnastesiPerPasien
DROP PROCEDURE IF EXISTS `LaporanJasaAnastesiPerPasien`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaAnastesiPerPasien`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
	  ...
	  'SELECT INST.NAMAINST, INST.ALAMATINST, NOPEN,UNITPELAYANAN, NORM, NAMAPASIEN, TANGGALREGISTRASI, TANGGALTINDAKAN, NAMATINDAKAN, JUMLAH
			 ...
			 FROM (
						  SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
                              ...
                              , master.getNamaLengkapPegawai(mpdokanas.NIP) DOKANASTESI
							...
							GROUP BY ptmanas.MEDIS, kjgn.RUANGAN, kjgn.NOPEN, tm.TINDAKAN
							UNION
					/*Paket*/
					SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
										, INSERT(INSERT(INSERT(LPAD(ps.NORM,8,''0''),3,0,''-''),6,0,''-''),9,0,''-'') NORM
					...
					GROUP BY MEDIS, IDRUANGAN, NOPEN,IDTINDAKAN
	');
