...
-- membuang struktur untuk procedure laporan.LaporanRL37
DROP PROCEDURE IF EXISTS `LaporanRL37`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanRL37`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME
)
BEGIN	
	...
	SET @sqlText = CONCAT('
		SELECT INST.*, rl.KODE, rl.DESKRIPSI, ''',TAHUN,''' TAHUN, SUM(IF(rl37.JUMLAH IS NULL,0,rl37.JUMLAH)) JUMLAH
