...
-- membuang struktur untuk procedure laporan.LaporanJasaDokterPerPasien1
DROP PROCEDURE IF EXISTS `LaporanJasaDokterPerPasien1`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaDokterPerPasien1`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
   ...
   SET vRUANGAN = CONCAT(RUANGAN,'%');
	...
	SET @sqlText = CONCAT(
	  'SELECT INST.NAMAINST, INST.ALAMATINST, NOPEN,UNITPELAYANAN, NORM, NAMAPASIEN, TANGGALREGISTRASI, TANGGALTINDAKAN, NAMATINDAKAN, JUMLAH
					 , CARABAYAR, PETUGASMEDIS, DOKANASTESI, master.getHeaderLaporan(''',RUANGAN,''') INSTALASI
					 ...
					 , SUM(TARIF) TARIF, SUM(DISKON) DISKON, SUM(DOKTER_OPERATOR)/100000 POINT, SUM(DOKTER_ANASTESI)/100000 POINT_ANASTESI
			 ...
			 FROM (
						  SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
									   ...
									   , master.getNamaLengkap(ps.NORM) NAMAPASIEN
										...
										, DATE_FORMAT(tm.TANGGAL,''%d-%m-%Y %H:%i:%s'') TANGGALTINDAKAN, tm.TINDAKAN IDTINDAKAN
										, t.NAMA NAMATINDAKAN, COUNT(tm.TINDAKAN) JUMLAH
										, crbyr.DESKRIPSI CARABAYAR, ptm.MEDIS
										, master.getNamaLengkapPegawai(mpdok.NIP) PETUGASMEDIS, master.getNamaLengkapPegawai(mpdokanas.NIP) DOKANASTESI
										...
										, SUM(mtt.TARIF) TARIF
										, SUM(IF(ds.TOTAL IS NULL,0,ds.TOTAL)) DISKON
							FROM pembayaran.rincian_tagihan rt
							     ...
							     LEFT JOIN `master`.tindakan t ON t.ID = tm.TINDAKAN
								  ...
								  LEFT JOIN layanan.petugas_tindakan_medis ptmanas ON tm.ID=ptmanas.TINDAKAN_MEDIS AND ptmanas.JENIS=2 AND ptmanas.KE=1 AND ptm.STATUS=1
							...
							WHERE tm.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND tm.STATUS=1
							  AND rt.JENIS = 3 AND 
                rt.`STATUS` NOT IN (0) AND mtt.DOKTER_OPERATOR !=0
							...
							GROUP BY ptm.MEDIS, kjgn.RUANGAN, kjgn.NOPEN, tm.TINDAKAN, tm.TANGGAL
							UNION
					/*Paket*/
				SELECT RAND() QID, kjgn.NOPEN, r.DESKRIPSI UNITPELAYANAN, kjgn.RUANGAN IDRUANGAN
										, INSERT(INSERT(INSERT(LPAD(ps.NORM,8,''0''),3,0,''-''),6,0,''-''),9,0,''-'') NORM
							...
							WHERE tm.TANGGAL BETWEEN ''',TGLAWAL,''' AND ''',TGLAKHIR,''' AND  tm.STATUS=1
							  AND  rt.`STATUS` NOT IN (0) AND mtt.DOKTER_OPERATOR !=0
							  ',IF(RUANGAN=0,'',CONCAT(' AND kjgn.RUANGAN LIKE ''',vRUANGAN,'''')),' 
								',IF(CARABAYAR=0,'',CONCAT(' AND pj.JENIS=',CARABAYAR)),'
								',IF(DOKTER=0,'',CONCAT(' AND ptm.MEDIS=',DOKTER)),'
							GROUP BY ptm.MEDIS, kjgn.RUANGAN, kjgn.NOPEN, tm.TINDAKAN,tm.TANGGAL
					...
					GROUP BY MEDIS, IDRUANGAN, NOPEN,IDTINDAKAN, TANGGALTINDAKAN
	');
