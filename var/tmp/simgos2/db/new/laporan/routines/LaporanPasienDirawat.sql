...
-- membuang struktur untuk procedure laporan.LaporanPasienDirawat
DROP PROCEDURE IF EXISTS `LaporanPasienDirawat`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanPasienDirawat`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT)
BEGIN
 ...
 SET @sqlText = CONCAT('
	SELECT INST.NAMAINST, INST.ALAMATINST, CONCAT(''LAPORAN PASIEN DIRAWAT ('',UPPER(jk.DESKRIPSI),'')'') JENISLAPORAN,
		    ...
		    pp.TANGGAL TGLMASUK, r.DESKRIPSI UNIT, CONCAT(rk.KAMAR,'' / '',kelas.DESKRIPSI) KAMAR
	...
	ORDER BY pk.RUANGAN, CONCAT(rk.KAMAR,'' / '',kelas.DESKRIPSI)
	');
