...
-- membuang struktur untuk procedure laporan.LaporanJasaAsistenPerPasienPalu
DROP PROCEDURE IF EXISTS `LaporanJasaAsistenPerPasienPalu`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanJasaAsistenPerPasienPalu`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `CARABAYAR` INT, IN `DOKTER` INT)
BEGIN
				...
				FROM pembayaran.pembayaran_tagihan pt
				   , pembayaran.rincian_tagihan rt
				     ...
				     LEFT JOIN master.tindakan_keperawatan tk ON tm.TINDAKAN=tk.TINDAKAN AND tk.`STATUS`=1
				  	  LEFT JOIN layanan.petugas_tindakan_medis ptm ON tm.ID=ptm.TINDAKAN_MEDIS AND ptm.JENIS=1  AND ptm.KE=2 AND ptm.STATUS=1
