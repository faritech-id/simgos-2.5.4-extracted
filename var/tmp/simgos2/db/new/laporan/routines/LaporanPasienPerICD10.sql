...
-- membuang struktur untuk procedure laporan.LaporanPasienPerICD10
DROP PROCEDURE IF EXISTS `LaporanPasienPerICD10`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanPasienPerICD10`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `CARAKELUAR` TINYINT, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT, IN `KODEICD` CHAR(6), IN `UTAMA` CHAR(50), IN `LMT` SMALLINT)
BEGIN
