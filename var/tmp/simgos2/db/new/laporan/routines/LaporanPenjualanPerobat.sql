...
-- membuang struktur untuk procedure laporan.LaporanPenjualanPerobat
DROP PROCEDURE IF EXISTS `LaporanPenjualanPerobat`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `LaporanPenjualanPerobat`(IN `TGLAWAL` DATETIME, IN `TGLAKHIR` DATETIME, IN `RUANGAN` CHAR(10), IN `LAPORAN` INT, IN `CARABAYAR` INT, IN `KATEGORI` INT, IN `BARANG` INT)
BEGIN
   ...
   SET vKATEGORI = CONCAT(KATEGORI,'%');
	...
	SET @sqlText = CONCAT('
		...
		FROM (
						  ...
						  (SUM(ppd.JUMLAH) - IF((SELECT SUM(JUMLAH) 
						   ...
						   (SUM(ppd.JUMLAH) - IF((SELECT SUM(JUMLAH) 
								...
								WHERE prp.PENJUALAN_ID=ppd.PENJUALAN_ID 
						  		...
						  		FROM penjualan.retur_penjualan prp 
								WHERE prp.PENJUALAN_ID=ppd.PENJUALAN_ID 
									AND prp.BARANG=ppd.BARANG))) * ppd.HARGA_BARANG TOTALJUAL, pt.TOTAL, ppd.HARGA_BARANG HARGA_JUAL,
			...
			GROUP BY pp.NOMOR, ppd.BARANG) a
	GROUP BY ID
		');
