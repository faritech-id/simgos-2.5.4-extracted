...
-- membuang struktur untuk procedure laporan.SensusHarianPasienKeluarHidup
DROP PROCEDURE IF EXISTS `SensusHarianPasienKeluarHidup`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `SensusHarianPasienKeluarHidup`(IN `TGLAWAL` DATETIME, IN `RUANGAN` CHAR(10))
BEGIN
 ...
 SET @sqlText = CONCAT('
	...
	WHERE lpp.KUNJUNGAN=pk.NOMOR AND lpp.`STATUS`=1 AND lpp.CARA NOT IN (6,7)
