...
-- membuang struktur untuk function pembayaran.getSkripRincianTagihanObat
DROP FUNCTION IF EXISTS `getSkripRincianTagihanObat`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` FUNCTION `getSkripRincianTagihanObat`(
	`PTAGIHAN` CHAR(10)
...
BEGIN
	...
	SELECT RAND() QID, rt.TAGIHAN, rt.REF_ID, 
			 ...
			 1 JUMLAH, SUM(rt.JUMLAH*rt.TARIF) TARIF, rt.`STATUS`, r.JENIS_KUNJUNGAN
	  FROM pembayaran.rincian_tagihan rt
