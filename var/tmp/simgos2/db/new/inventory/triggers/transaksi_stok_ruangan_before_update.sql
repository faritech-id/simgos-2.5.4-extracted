...
-- membuang struktur untuk trigger inventory.transaksi_stok_ruangan_before_update
DROP TRIGGER IF EXISTS `transaksi_stok_ruangan_before_update`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
DELIMITER //
CREATE TRIGGER `transaksi_stok_ruangan_before_update` BEFORE UPDATE ON `transaksi_stok_ruangan` FOR EACH ROW BEGIN
	...
	IF NOT EXISTS(SELECT 1
			FROM inventory.transaksi_stok_ruangan
		  WHERE BARANG_RUANGAN = OLD.BARANG_RUANGAN
			 AND ID > OLD.ID LIMIT 1) THEN
	BEGIN
		UPDATE inventory.barang_ruangan
		   SET STOK = NEW.STOK,
		   	 TANGGAL = NEW.TANGGAL,
		   	 TRANSAKSI_STOK_RUANGAN = OLD.ID
		 WHERE ID = OLD.BARANG_RUANGAN;
	END;
	END IF;
END//
DELIMITER ;
