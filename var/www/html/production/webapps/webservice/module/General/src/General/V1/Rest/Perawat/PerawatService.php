...
class PerawatService extends Service
{
	...
	public function cariPerawat($params = []) {
		...
		if(isset($params["NIP"])) {
			$pers = $this->load(["NIP" => $params["NIP"]]);		
			if(count($pers) > 0) $perawats[] = $pers[0];
		} else { 
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		...
		if(!System::isNull($params, 'NAMA')) { 
			...
			$select->where->like('p.NAMA', '%'.$params['NAMA'].'%');
			unset($params['NAMA']);
		}
		
		if(isset($params['QUERY'])) if(!System::isNull($params, 'QUERY')) {
			$select->join(['p' => new TableIdentifier("pegawai", "master")], "perawat.NIP = p.NIP", ["NIP", "NAMA", "GELAR_DEPAN", "GELAR_BELAKANG"]);
			$params[] = new \Laminas\Db\Sql\Predicate\Expression("(p.NAMA LIKE ? OR p.NIP LIKE ?)", ["%".$params["QUERY"]."%", $params["QUERY"]."%"]);
			unset($params['QUERY']);
		}
	}
