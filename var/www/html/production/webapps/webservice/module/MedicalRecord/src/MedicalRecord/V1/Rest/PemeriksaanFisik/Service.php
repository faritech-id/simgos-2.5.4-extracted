...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = []) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\PemeriksaanFisik\\PemeriksaanFisikEntity";
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		...
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['pemeriksaan_fisik.ID'] = $id;
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(['k'=>new TableIdentifier('kunjungan', 'pendaftaran')], 'k.NOMOR = pemeriksaan_fisik.KUNJUNGAN', []);
