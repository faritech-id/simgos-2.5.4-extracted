...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = []) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\TindakanMMPI\\TindakanMMPIEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("tindakan_mmpi", "medicalrecord"));
		$this->entity = new TindakanMMPIEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			$params['tindakan_mmpi.STATUS'] = $params['STATUS'];
			unset($params['STATUS']);
		}

		if(!System::isNull($params, 'ID')) {
			$params['tindakan_mmpi.ID'] = $params['ID'];
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(['k'=>new TableIdentifier('kunjungan', 'pendaftaran')], 'k.NOMOR = tindakan_mmpi.KUNJUNGAN', []);
