...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = []) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\RiwayatLainnya\\RiwayatLainnyaEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("riwayat_lainnya", "medicalrecord"));
		$this->entity = new RiwayatLainnyaEntity();
		...
		if($this->includeReferences) {
			foreach($data as &$entity) {
				...
				if(!empty($entity['JENIS'])){ 
					$referensi = $this->referensi->load(['IN_ID' => $entity['JENIS'], 'JENIS'=>180]);
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['riwayat_lainnya.STATUS'] = $status;
			unset($params['STATUS']);
		}

		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['riwayat_lainnya.ID'] = $id;
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(['k'=>new TableIdentifier('kunjungan', 'pendaftaran')], 'k.NOMOR = riwayat_lainnya.KUNJUNGAN', []);
