...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = []) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\SuratSehat\\SuratSehatEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("surat_sehat", "medicalrecord"));
		$this->entity = new SuratSehatEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['surat_sehat.STATUS'] = $status;
			unset($params['STATUS']);
		}
		
		if(!System::isNull($params, 'TAHUN')) {
			$tahun = $params['TAHUN'];
			$select->where("(YEAR(surat_sehat.TANGGAL) = '".$tahun."')");
			unset($params['TAHUN']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(['k'=>new TableIdentifier('kunjungan', 'pendaftaran')], 'k.NOMOR = surat_sehat.KUNJUNGAN', []);
	...
	public function generateNomor() {
	    ...
	    $result = $conn->execute("SELECT generator.generateNoSuratSehat(YEAR(NOW())) NOMOR");
        $data = $result->current();
        return $data["NOMOR"];
    }
}