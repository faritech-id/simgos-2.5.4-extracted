...
class Service extends DBService {	
	...
	public function __construct($includeReferences = true, $references = array()) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\AnamnesisDiPeroleh\\AnamnesisDiPerolehEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("anamnesis_diperoleh", "medicalrecord"));
		$this->entity = new AnamnesisDiPerolehEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['anamnesis_diperoleh.STATUS'] = $status;
			unset($params['STATUS']);
		}
		
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['anamnesis_diperoleh.ID'] = $id;
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(array('k'=>new TableIdentifier('kunjungan', 'pendaftaran')), 'k.NOMOR = anamnesis_diperoleh.KUNJUNGAN', array());
