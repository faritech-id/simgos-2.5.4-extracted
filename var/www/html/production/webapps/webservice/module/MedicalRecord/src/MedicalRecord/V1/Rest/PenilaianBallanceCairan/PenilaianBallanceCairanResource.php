...
class PenilaianBallanceCairanResource extends Resource
{
    protected $title = "Penilaian Ballance Cairan";
    ...
    public function fetchAll($params = [])
    {
        ...
        if(isset($params["GETTOTAL"])){
            $data = $this->service->execute("CALL medicalrecord.`getTotalBallancerCairan`(?)", [$params["NOPEN"]]);
            $total = count($data);
        } else{                 
            ...
            if (isset($params["NOT"])) {
                ...
                foreach($nots as $key => $val) {                
                    $params[] = new \Laminas\Db\Sql\Predicate\Expression("(NOT penilaian_ballance_cairan.".$key." = ".$val.")");                
                }
                unset($params["NOT"]);
            }
