...
class Service extends DBService {	
	...
	public function __construct($includeReferences = true, $references = array()) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\RiwayatPenyakitKeluarga\\RiwayatPenyakitKeluargaEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("riwayat_penyakit_keluarga", "medicalrecord"));
		$this->entity = new RiwayatPenyakitKeluargaEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['riwayat_penyakit_keluarga.STATUS'] = $status;
			unset($params['STATUS']);
		}
		
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['riwayat_penyakit_keluarga.ID'] = $id;
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(array('k'=>new TableIdentifier('kunjungan', 'pendaftaran')), 'k.NOMOR = riwayat_penyakit_keluarga.KUNJUNGAN', array());
