...
class Service extends DBService
{
    ...
    public function __construct() {
        $this->table = DatabaseService::get('SIMpel')->get(new TableIdentifier("rekonsiliasi_transfer_detil", "medicalrecord"));
		$this->entity = new RekonsiliasiTransferDetilEntity();
	...
	protected function query($columns, $params, $isCount = false, $orders = array()) {		
		...
		return $this->table->select(function(Select $select) use ($isCount, $params, $columns, $orders) {
			...
			if(isset($params['STATUS'])) if(!System::isNull($params, 'STATUS')){
				$select->where("(rekonsiliasi_transfer_detil.STATUS IN (1, 2))");
				unset($params['STATUS']);
			}
	...
	public function getDataRekonsiliasiTransferDetil($data){
		$adapter = $this->table->getAdapter();
		$strLmt = $adapter->query("SELECT a.ID IDREKONSILIASITRANSFERDETIL, a.OBAT_DARI_LUAR, ib.NAMA NAMA_OBAT_DARI_LUAR, a.DOSIS, a.FREKUENSI, far.FREKUENSI DESKRIPSI_FREKUENSI, a.RUTE, ref.DESKRIPSI DESKRIPSI_RUTE
									, a.TINDAK_LANJUT, ref2.DESKRIPSI DESKRIPSI_TINDAK_LANJUT, a.PERUBAHAN_ATURAN_PAKAI, pak.FREKUENSI DESKRIPSI_PERUBAHAN_ATURAN_PAKAI, a.REKONSILIASI_ADMISI_DETIL IDREKONSILIASIADMISIDETIL
									, a.LAYANAN_FARMASI
									FROM medicalrecord.rekonsiliasi_transfer_detil a
									JOIN medicalrecord.rekonsiliasi_transfer b ON a.REKONSILIASI_TRANSFER=b.ID
									...
									WHERE d.NORM=? AND b.STATUS=2 AND c.NOMOR=(SELECT c1.NOMOR 
											FROM medicalrecord.rekonsiliasi_transfer b1 
