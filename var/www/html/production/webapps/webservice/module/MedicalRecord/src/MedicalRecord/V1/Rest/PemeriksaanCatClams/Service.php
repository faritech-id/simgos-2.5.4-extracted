...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = []) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\PemeriksaanCatClams\\PemeriksaanCatClamsEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("pemeriksaan_cat_clams", "medicalrecord"));
		$this->entity = new PemeriksaanCatClamsEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['pemeriksaan_cat_clams.STATUS'] = $status;
			unset($params['STATUS']);
		}

		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['pemeriksaan_cat_clams.ID'] = $id;
			unset($params['ID']);
		}
		
		if(!System::isNull($params, 'TAHUN')) {
			$tahun = $params['TAHUN'];
			$select->where("(YEAR(pemeriksaan_cat_clams.DIBUAT_TANGGAL) = '".$tahun."')");
			unset($params['TAHUN']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(['k'=>new TableIdentifier('kunjungan', 'pendaftaran')], 'k.NOMOR = pemeriksaan_cat_clams.KUNJUNGAN', []);
