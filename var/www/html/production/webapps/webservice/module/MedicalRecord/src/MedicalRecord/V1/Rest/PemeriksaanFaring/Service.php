...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = []) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\PemeriksaanFaring\\PemeriksaanFaringEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("pemeriksaan_faring", "medicalrecord"));
		$this->entity = new PemeriksaanFaringEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['pemeriksaan_faring.STATUS'] = $status;
			unset($params['STATUS']);
		}

		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['pemeriksaan_faring.ID'] = $id;
			unset($params['ID']);
		}
		
		if(!System::isNull($params, 'TAHUN')) {
			$tahun = $params['TAHUN'];
			$select->where("(YEAR(pemeriksaan_faring.DIBUAT_TANGGAL) = '".$tahun."')");
			unset($params['TAHUN']);
		}
		...
		if($this->includeReferences) {
			if($this->references['Referensi']) {
				...
				$select->join(
					...
					new \Laminas\Db\Sql\Predicate\Expression("kelainan.ID = pemeriksaan_faring.ADA_KELAINAN AND kelainan.JENIS = 178"),
					$fields,
					Select::JOIN_LEFT
				);
			}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(['k'=>new TableIdentifier('kunjungan', 'pendaftaran')], 'k.NOMOR = pemeriksaan_faring.KUNJUNGAN', []);
