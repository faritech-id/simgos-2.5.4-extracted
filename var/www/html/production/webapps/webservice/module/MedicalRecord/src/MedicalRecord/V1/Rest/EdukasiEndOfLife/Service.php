...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = array()) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\EdukasiEndOfLife\\EdukasiEndOfLifeEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("edukasi_end_of_life", "medicalrecord"));
		$this->entity = new EdukasiEndOfLifeEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['edukasi_end_of_life.STATUS'] = $status;
			unset($params['STATUS']);
		}
		
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['edukasi_end_of_life.ID'] = $id;
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) { 
			...
			$select->join(array('k'=>new TableIdentifier('kunjungan', 'pendaftaran')), 'k.NOMOR = edukasi_end_of_life.KUNJUNGAN', array());
