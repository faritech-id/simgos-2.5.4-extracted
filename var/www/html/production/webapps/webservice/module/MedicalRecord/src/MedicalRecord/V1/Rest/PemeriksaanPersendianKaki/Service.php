...
class Service extends DBService {
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		...
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['pemeriksaan_persendian_kaki.ID'] = $id;
			unset($params['ID']);
		}
		...
		if($this->includeReferences) {
			if($this->references['Referensi']) {
				...
				$select->join(
					...
					new \Laminas\Db\Sql\Predicate\Expression("kelainan.ID = pemeriksaan_persendian_kaki.ADA_KELAINAN AND kelainan.JENIS = 178"),
					$fields,
					Select::JOIN_LEFT
				);
			}
