...
class Service extends DBService {	
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		...
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['status_pediatric.ID'] = $id;
			unset($params['ID']);
		}

		if(!System::isNull($params, 'TAHUN')) {
			$tahun = $params['TAHUN'];
			$select->where("(YEAR(status_pediatric.DIBUAT_TANGGAL) = '".$tahun."')");
			unset($params['TAHUN']);
		}
		
		if($this->includeReferences) {
			if($this->references['Referensi']) {
				$fields = $this->referensi->getEntity()->getFieldWithAlias("gizi");
				$select->join(
					["gizi" => new TableIdentifier('referensi', 'master')],
					new \Laminas\Db\Sql\Predicate\Expression("gizi.ID = status_pediatric.STATUS_PEDIATRIC AND gizi.JENIS = 155"),
					$fields,
					Select::JOIN_LEFT
				);
			}
	...
	public function load($params = [], $columns = ['*'], $orders = []) {
		...
		if($this->includeReferences) {
			foreach($data as &$entity) {
				if($this->references['Referensi']) {
					$results = $this->parseReferensi($entity, "gizi");
					if(count($results) > 0) $entity["REFERENSI"]["STATUS_PEDIATRIC"] = $results;
				}
