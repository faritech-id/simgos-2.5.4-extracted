...
class Service extends DBService {	
	...
	public function __construct($includeReferences = true, $references = array()) {
		$this->config["entityName"] = "MedicalRecord\\V1\\Rest\\HubunganPsikososialEndOfLife\\HubunganPsikososialEndOfLifeEntity";
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("hubungan_psikososial_end_of_life", "medicalrecord"));
		$this->entity = new HubunganPsikososialEndOfLifeEntity();
	...
	protected function queryCallback(Select &$select, &$params, $columns, $orders) {
		if(!System::isNull($params, 'STATUS')) {
			...
			$params['hubungan_psikososial_end_of_life.STATUS'] = $status;
			unset($params['STATUS']);
		}
		
		if(!System::isNull($params, 'ID')) {
			$id = $params['ID'];
			$params['hubungan_psikososial_end_of_life.ID'] = $id;
			unset($params['ID']);
		}
		...
		if(!System::isNull($params, 'HISTORY')) {
			...
			$select->join(array('k'=>new TableIdentifier('kunjungan', 'pendaftaran')), 'k.NOMOR = hubungan_psikososial_end_of_life.KUNJUNGAN', array());
