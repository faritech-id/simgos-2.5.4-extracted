...
class Generator
{
	...
	public static function generateNoOrderPermintaanDarah($ruangan) {
		$stmt = self::$adapter->query('SELECT generator.generateNoOrderPermintaanDarah('.$ruangan.', DATE(NOW())) ID');
		$rst = $stmt->execute();
		$row = $rst->current();
	...
	public static function generateIdHasilEvaluasiSST() {
		$stmt = self::$adapter->query('SELECT generator.generateIdHasilEvaluasiSST(DATE(NOW())) ID');
		$rst = $stmt->execute();
		$row = $rst->current();
	...
	public static function generateNoKomplain() {
		$stmt = self::$adapter->query('SELECT generator.generateNoKomplain(DATE(NOW())) ID');
		$rst = $stmt->execute();
		$row = $rst->current();
