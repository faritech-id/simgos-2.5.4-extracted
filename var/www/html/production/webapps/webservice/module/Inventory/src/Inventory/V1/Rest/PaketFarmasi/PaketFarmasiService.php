		...
		return $this->table->select(function(Select $select) use ($isCount, $params, $columns, $orders) {
			...
			if(isset($params['STATUS'])) {
				if(!System::isNull($params, 'STATUS')) { 
					$select->where(["barang_paket_tindakan.STATUS" => $params['STATUS']]);
					unset($params['STATUS']);
				}
			}
			if(!System::isNull($params, 'TINDAKAN_PAKET')) { 
				$select->join(['d' => new TableIdentifier("tindakan_paket", "master")], "barang_paket_tindakan.TINDAKAN = d.ID", ['TINDAKAN_PAKET' => 'TINDAKAN']);
				$select->where(["d.TINDAKAN" => $params['TINDAKAN_PAKET']]);
				unset($params['TINDAKAN_PAKET']);
			}
			$select->where($params);
			$select->order($orders);
		})->toArray();
